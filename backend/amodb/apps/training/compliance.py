...
def build_status_item_from_dates(
    *,
    ...
    return training_schemas.TrainingStatusItem(
        ...
        is_mandatory=bool(getattr(course, "is_mandatory", False) or getattr(course, "mandatory_for_all", False)),
        group_code=getattr(course, "group_code", None),
        prerequisite_course_id=getattr(course, "prerequisite_course_id", None),
        kind=str(getattr(getattr(course, "kind", None), "value", getattr(course, "kind", None)) or getattr(course, "status", None) or None),
