...
class TrainingAssessmentAttemptPolicy(Base):
    """Explicit tenant policy for one canonical assessment-template revision.

    A policy row must exist before a learner can start an attempt. Nullable time
    limit means the tenant explicitly chose no countdown; absence of the row never
    means a portal default.
    """
