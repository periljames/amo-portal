...
def reader_manifest(db: Session, tenant: manual_models.Tenant, manual: manual_models.Manual, revision: manual_models.ManualRevision) -> dict[str, Any]:
    ...
    pdf = source_type == "PDF" or mime == "application/pdf"
    if pdf:
        renderer, adapter, selection = "PDF_V3", "PDF_CANONICAL_PAGE", "NATIVE_TEXT_IF_PRESENT"
    elif section_count:
        renderer, adapter, selection = "SEMANTIC_HTML", "SEMANTIC_SECTION_BLOCK", "SEMANTIC_TEXT"
    else:
        renderer, adapter, selection = "DOWNLOAD_ONLY", "FILE_IDENTITY_ONLY", "NONE"
    ...
    return {
        ...
        "capabilities": {
            "layout": pdf,
            "semantic_text": bool(section_count),
