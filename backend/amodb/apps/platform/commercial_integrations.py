...
def install_commercial_integrations() -> None:
    ...
    _register_provider(
        saas_providers.ProviderDefinition(
            ...
            "OAuth-linked QuickBooks Online accounting export for portal invoices and settlements. OAuth access/refresh tokens are written only by the verified callback/refresh path, never by the configuration form.",
        )
    )
