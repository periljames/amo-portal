...
from amodb.database import Base, WriteSessionLocal, write_engine
...
def test_first_party_module_graph_has_explicit_commercial_boundaries() -> None:
    ...
    assert set(catalog["supply_chain_finance_suite"]["included_modules"]) == {"finance", "inventory", "procurement"}
...
def test_tenant_frontend_has_no_manual_card_or_payment_token_entry() -> None:
    billing = (FRONTEND / "pages" / "SubscriptionManagementPage.tsx").read_text(encoding="utf-8")
    lowered = billing.lower()
    for forbidden in [
        'id="externalref"',
        'id="last4"',
        'id="expmonth"',
    ...
    assert "hosted card/bank checkout" in lowered
    assert "never asks for or stores a card number" in lowered


def test_tenant_billing_cure_route_is_not_limited_to_general_admin_guard() -> None:
    route_surface = (FRONTEND / "app" / "PortalRouteSurface.tsx").read_text(encoding="utf-8")
    billing = (FRONTEND / "pages" / "SubscriptionManagementPage.tsx").read_text(encoding="utf-8")
    assert 'path="/maintenance/:amoCode/admin/billing"' in route_surface
    assert "<TenantBillingPage />" in route_surface
    assert "RequireTenantAdmin" not in route_surface
    assert "FINANCE_MANAGER" in billing
    assert "ACCOUNTS_OFFICER" in billing
    assert "purchaseSubscription" not in billing
    assert "addPaymentMethod" not in billing


def _ensure_legacy_renewal_test_schema() -> None:
    """Create the minimal billing schema used by direct SessionLocal renewal tests.

    The shared pytest harness intentionally points the application SessionLocal at
    an in-memory SQLite database, so tests that bypass the ``db_session`` fixture
    must explicitly create the tables they exercise.
    """
    Base.metadata.create_all(
        ...
        tables=[
            ...
            account_models.BillingAuditLog.__table__,
...
def _create_elapsed_paid_legacy_case() -> tuple[object, str, str, str, datetime]:
    _ensure_legacy_renewal_test_schema()
...
def test_paid_legacy_period_creates_renewal_invoice_and_locks() -> None:
    ...
    try:
        ...
        assert details.get("source") == "BASE_RENEWAL"
        assert details.get("lock_scope") == "ACCOUNT"
    finally:
        db.close()


def test_verified_legacy_renewal_settlement_reactivates_license(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    try:
        ...
        # This test owns the base-license renewal transition. Provider billing
        # accounts and the platform-wide audit table are covered by their own
        # contract suites and are intentionally outside this minimal SQLite schema.
        monkeypatch.setattr(commercial_services, "_billing_account", lambda *args, **kwargs: None)
        monkeypatch.setattr(commercial_services, "_audit", lambda *args, **kwargs: None)
