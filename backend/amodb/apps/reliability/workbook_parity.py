...
DATASET_CATALOG: dict[WorkbookDatasetCode, DatasetDefinition] = {
    WorkbookDatasetCode.AU: DatasetDefinition(
        code=WorkbookDatasetCode.AU, name="Aircraft utilisation", workbook_sheet_names=["AU", "AIRCRAFT UTILIZATION", "AIRCRAFT UTILISATION"],
        description="Daily aircraft, engine and APU exposure used as controlled Reliability denominators.",
        fields=[
            _field("flight_hours", "Aircraft flight hours", "decimal", required=True, unit="FH"),
            _field("flight_cycles", "Aircraft flight cycles", "integer", required=True, unit="FC"),
            _field("landings", "Landings", "integer", unit="LDG"),
            _field("engine_1_hours", "Engine 1 hours", "decimal", unit="EH"), _field("engine_1_cycles", "Engine 1 cycles", "integer"),
            _field("engine_2_hours", "Engine 2 hours", "decimal", unit="EH"), _field("engine_2_cycles", "Engine 2 cycles", "integer"),
            _field("apu_hours", "APU hours", "decimal", unit="AH"), _field("apu_cycles", "APU cycles", "integer"),
            _field("source_reference", "Logbook / utilisation reference", required=True), _field("remarks", "Remarks", "textarea"),
        ],
    ),
    WorkbookDatasetCode.AI: DatasetDefinition(
        code=WorkbookDatasetCode.AI, name="Aircraft incidents", workbook_sheet_names=["AI", "AIRCRAFT INCIDENTS"],
        description="Aircraft incident and serious technical-event evidence with investigation and operational consequence fields.", event_type="SAFETY_EVENT",
        fields=[
            _field("incident_number", "Incident number", required=True), _field("event_type", "Incident type", "select", required=True, options=["ACCIDENT", "SERIOUS_INCIDENT", "INCIDENT", "GROUND_DAMAGE", "OTHER"]),
            _field("occurred_at", "Occurrence date and time", "datetime", required=True), _field("location", "Location / station", required=True),
            _field("phase_of_flight", "Phase of flight", "select", options=["GROUND", "TAXI", "TAKEOFF", "CLIMB", "CRUISE", "DESCENT", "APPROACH", "LANDING", "MAINTENANCE"]),
            _field("component_part_number", "Component part number"), _field("component_serial_number", "Component serial number"),
            _field("damage_description", "Aircraft / component damage", "textarea"), _field("injury_information", "Injury information", "textarea"),
            _field("operational_consequence", "Operational consequence", "textarea"), _field("investigation_reference", "Investigation reference"),
            _field("action_taken", "Immediate / corrective action", "textarea", required=True), _field("severity", "Severity", "select", required=True, options=["LOW", "MEDIUM", "HIGH", "CRITICAL"]),
        ],
    ),
    WorkbookDatasetCode.PM: DatasetDefinition(
        code=WorkbookDatasetCode.PM, name="Pilot and maintenance reports", workbook_sheet_names=["PM", "PILOT REPORTS", "MAINTENANCE REPORTS", "PIREP", "MAREP"],
        description="Technical-log pilot, cabin and maintenance reports with rectification, component and recurrence evidence.", event_type="PILOT_REPORT",
        fields=[
            _field("report_type", "Report type", "select", required=True, options=["PILOT", "MAINTENANCE", "CABIN"]), _field("logbook_reference", "Technical log reference", required=True),
            _field("flight_number", "Flight number"), _field("station", "Station"), _field("defect_description", "Defect / report", "textarea", required=True),
            _field("action_taken", "Maintenance action", "textarea", required=True), _field("deferred_reference", "MEL/CDL / deferred reference"),
            _field("component_part_number", "Part number"), _field("component_serial_number", "Serial number"), _field("work_order_reference", "Work order reference"),
            _field("repeat_key", "Controlled recurrence key"), _field("is_repeat", "Repeat report", "boolean"), _field("closed_date", "Closure date", "date"),
        ],
    ),
    WorkbookDatasetCode.OOS: DatasetDefinition(
        code=WorkbookDatasetCode.OOS, name="Aircraft out of service", workbook_sheet_names=["OS", "OOS", "OUT OF SERVICE"],
        description="Out-of-service intervals, downtime cause, availability exposure, restoration evidence and MTTR inputs.", event_type="OTHER",
        fields=[
            _field("start_at", "Out-of-service start", "datetime", required=True), _field("end_at", "Returned-to-service time", "datetime"),
            _field("reason_category", "Reason category", "select", required=True, options=["TECHNICAL", "SCHEDULED_MAINTENANCE", "UNSCHEDULED_MAINTENANCE", "PARTS", "MANPOWER", "FACILITY", "OTHER"]),
            _field("defect_reference", "Defect reference"), _field("work_order_reference", "Work order / workpack reference"),
            _field("maintenance_action", "Maintenance action", "textarea", required=True), _field("scheduled_available_hours", "Scheduled available hours", "decimal", unit="h"),
            _field("cancelled_flights", "Cancelled flights", "integer"), _field("delay_minutes", "Associated delay minutes", "integer", unit="min"),
            _field("release_reference", "Return-to-service / CRS reference"), _field("responsible_area", "Responsible area"),
        ],
    ),
    WorkbookDatasetCode.RM: DatasetDefinition(
        code=WorkbookDatasetCode.RM, name="Component removals", workbook_sheet_names=["RM", "COMPONENT REMOVALS", "REMOVALS"],
        description="Complete off/on component identity, removal classification, life at removal and disposition evidence.", event_type="UNSCHEDULED_REMOVAL",
        fields=[
            _field("removed_at", "Removal date and time", "datetime", required=True), _field("position", "Installed position"), _field("component_description", "Component description", required=True),
            _field("off_part_number", "Removed part number", required=True), _field("off_serial_number", "Removed serial number", required=True),
            _field("on_part_number", "Installed part number"), _field("on_serial_number", "Installed serial number"),
            _field("removal_type", "Removal type", "select", required=True, options=["SCHEDULED", "UNSCHEDULED", "ROBBERY", "MODIFICATION", "TEST"]),
            _field("reason_code", "Removal reason / failure mode", required=True), _field("confirmed_failure", "Confirmed failure", "boolean"),
            _field("hours_at_removal", "Component hours at removal", "decimal", unit="h"), _field("cycles_at_removal", "Component cycles at removal", "integer", unit="cy"),
            _field("time_since_new", "Time since new", "decimal", unit="h"), _field("cycles_since_new", "Cycles since new", "integer"),
            _field("shop_order_reference", "Shop order reference"), _field("vendor", "Vendor / repair agency"), _field("disposition", "Disposition", "textarea"),
        ],
    ),
    WorkbookDatasetCode.SM: DatasetDefinition(
        code=WorkbookDatasetCode.SM, name="Scheduled maintenance findings", workbook_sheet_names=["SM", "SCHEDULED MAINTENANCE"],
        description="Findings raised during scheduled checks with workpack, task, programme-item and closure provenance.", event_type="DEFECT",
        fields=[
            _field("check_type", "Check / maintenance package", required=True), _field("workpack_reference", "Workpack reference", required=True),
            _field("task_card_reference", "Task card reference"), _field("programme_item_reference", "AMP / programme item reference"),
            _field("finding_classification", "Finding classification", "select", required=True, options=["ROUTINE", "NON_ROUTINE", "STRUCTURAL", "CORROSION", "FUNCTIONAL_FAILURE", "OTHER"]),
            _field("finding_description", "Finding description", "textarea", required=True), _field("corrective_action", "Corrective action", "textarea", required=True),
            _field("component_part_number", "Part number"), _field("component_serial_number", "Serial number"), _field("man_hours", "Finding man-hours", "decimal", unit="MH"),
            _field("deferred_reference", "Deferral reference"), _field("close_date", "Closure date", "date"), _field("release_reference", "Release reference"),
        ],
    ),
    WorkbookDatasetCode.STRUCTURES: DatasetDefinition(
        code=WorkbookDatasetCode.STRUCTURES, name="Aircraft structures", workbook_sheet_names=["AIRCRAFT STRUCTURES", "STRUCTURES", "STRUCTURAL REPORTS"],
        description="Structural damage, location, limits, repair classification, repeat inspections and permanent-record evidence.", event_type="DEFECT",
        fields=[
            _field("damage_reference", "Damage / dent reference", required=True), _field("zone", "Aircraft zone"), _field("station", "Fuselage / wing station"),
            _field("frame", "Frame"), _field("stringer", "Stringer"), _field("skin_panel", "Skin / panel"), _field("damage_type", "Damage type", "select", required=True, options=["DENT", "CRACK", "CORROSION", "GOUGE", "PUNCTURE", "DELAMINATION", "OTHER"]),
            _field("dimensions", "Damage dimensions", required=True), _field("allowable_limits_reference", "SRM / approved limits reference", required=True),
            _field("repair_class", "Repair classification", "select", required=True, options=["WITHIN_LIMITS", "TEMPORARY_REPAIR", "PERMANENT_REPAIR", "OEM_DISPOSITION_REQUIRED"]),
            _field("repair_reference", "Repair / engineering order reference"), _field("inspection_interval", "Repeat inspection interval"),
            _field("next_inspection_due", "Next inspection due", "date"), _field("status", "Structural item status", "select", required=True, options=["OPEN", "MONITORING", "REPAIRED", "CLOSED"]),
            _field("evidence_reference", "Photo / drawing / NDT evidence reference"), _field("description", "Structural description", "textarea", required=True),
        ],
    ),
    WorkbookDatasetCode.RECURRING: DatasetDefinition(
        code=WorkbookDatasetCode.RECURRING, name="Recurring defects", workbook_sheet_names=["RECURRING DEFECTS", "REPEAT DEFECTS"],
        description="Dedicated recurrence groups with controlled repeat key, occurrence history, corrective action and effectiveness evidence.", event_type="REPEAT_DEFECT",
        fields=[
            _field("repeat_key", "Controlled repeat key", required=True), _field("first_seen", "First occurrence", "date", required=True), _field("last_seen", "Latest occurrence", "date", required=True),
            _field("origin", "Origin / source", "select", required=True, options=["PIREP", "MAREP", "SCHEDULED_MAINTENANCE", "COMPONENT_REMOVAL", "SHOP", "OTHER"]),
            _field("defect_description", "Recurring defect", "textarea", required=True), _field("occurrence_count", "Occurrence count", "integer", required=True),
            _field("recurrence_window_days", "Recurrence window", "integer", required=True, unit="days"), _field("corrective_action", "Corrective action", "textarea"),
            _field("part_number", "Part number"), _field("serial_number", "Serial number"), _field("work_order_reference", "Work order reference"),
            _field("fracas_case_id", "FRACAS case ID", "integer"), _field("effectiveness_result", "Effectiveness result", "select", options=["PENDING", "EFFECTIVE", "INEFFECTIVE"]),
            _field("status", "Recurrence status", "select", required=True, options=["OPEN", "UNDER_INVESTIGATION", "ACTIONED", "MONITORING", "CLOSED"]),
        ],
    ),
    WorkbookDatasetCode.ECTM: DatasetDefinition(
        code=WorkbookDatasetCode.ECTM, name="Engine condition and trend monitoring", workbook_sheet_names=["NC", "ECTM", "EHM", "ENGINE CONDITION"],
        description="Numeric engine trend parameters plus oil-analysis, borescope, analyst review and OEM recommendation status.", event_type="ECTM",
        fields=[
            _field("engine_position", "Engine position", required=True), _field("engine_serial_number", "Engine serial number"), _field("flight_leg", "Flight leg"),
            _field("phase", "Measurement phase"), _field("pressure_altitude_ft", "Pressure altitude", "decimal", unit="ft"), _field("oat_c", "Outside air temperature", "decimal", unit="°C"),
            _field("isa_dev_c", "ISA deviation", "decimal", unit="°C"), _field("power_reference", "Power reference", "decimal"),
            _field("itt_c", "ITT / EGT", "decimal", unit="°C"), _field("ng_pct", "Ng / N1", "decimal", unit="%"), _field("nh_pct", "Nh / N2", "decimal", unit="%"),
            _field("np_rpm", "Propeller speed", "decimal", unit="rpm"), _field("torque", "Torque", "decimal"), _field("fuel_flow", "Fuel flow", "decimal"),
            _field("oil_pressure", "Oil pressure", "decimal"), _field("oil_temperature", "Oil temperature", "decimal", unit="°C"), _field("vibration", "Vibration", "decimal"),
            _field("oil_analysis_status", "Oil analysis status", "select", options=["NOT_DUE", "PENDING", "NORMAL", "WATCH", "ALERT"]), _field("oil_analysis_reference", "Oil analysis report reference"),
            _field("borescope_status", "Borescope status", "select", options=["NOT_DUE", "PENDING", "SATISFACTORY", "MONITOR", "UNSATISFACTORY"]), _field("borescope_reference", "Borescope report reference"),
            _field("trend_status", "Trend status", "select", required=True, options=["NOT_EVALUATED", "NORMAL", "WATCH", "SHIFT", "ALERT"]),
            _field("analyst_comments", "Analyst comments", "textarea"), _field("oem_recommendation", "OEM recommendation", "textarea"),
            _field("review_date", "Review date", "date"), _field("reviewed_by", "Reviewed by"), _field("action_required", "Action required", "textarea"),
        ],
    ),
}
...
DEFAULT_LAYOUTS = [
    {
        "code": "C208B-RP",
        "name": "Cessna 208B Reliability Programme Report",
        "aircraft_family": "C208B",
        "sections": [
            {"code": "EXECUTIVE", "title": "Executive summary", "kind": "SUMMARY"},
            {"code": "AU", "title": "Aircraft utilisation", "kind": "DATASET", "dataset_code": "AU"},
            {"code": "EVENTS", "title": "Operational interruptions", "kind": "EVENTS"},
            {"code": "PM", "title": "Pilot and maintenance reports", "kind": "DATASET", "dataset_code": "PM"},
            {"code": "RM", "title": "Component removals", "kind": "DATASET", "dataset_code": "RM"},
            {"code": "OOS", "title": "Out-of-service and availability", "kind": "DATASET", "dataset_code": "OOS"},
            {"code": "RECURRING", "title": "Recurring defects", "kind": "DATASET", "dataset_code": "RECURRING"},
            {"code": "ECTM", "title": "Engine condition monitoring", "kind": "DATASET", "dataset_code": "ECTM"},
            {"code": "ALERTS", "title": "Statistical alert calculations", "kind": "STATISTICAL_ALERTS"},
        ],
    },
    {
        "code": "DHC8-RP",
        "name": "DHC8 Reliability Programme Report",
        "aircraft_family": "DHC8",
        "sections": [
            {"code": "EXECUTIVE", "title": "Executive summary", "kind": "SUMMARY"},
            {"code": "AU", "title": "Fleet and engine utilisation", "kind": "DATASET", "dataset_code": "AU"},
            {"code": "AI", "title": "Aircraft incidents", "kind": "DATASET", "dataset_code": "AI"},
            {"code": "EVENTS", "title": "Flight interruptions", "kind": "EVENTS"},
            {"code": "SM", "title": "Scheduled maintenance findings", "kind": "DATASET", "dataset_code": "SM"},
            {"code": "STRUCTURES", "title": "Structural damage and repairs", "kind": "DATASET", "dataset_code": "STRUCTURES"},
            {"code": "RM", "title": "Component removals", "kind": "DATASET", "dataset_code": "RM"},
            {"code": "OOS", "title": "Out-of-service and availability", "kind": "DATASET", "dataset_code": "OOS"},
            {"code": "RECURRING", "title": "Recurring defects", "kind": "DATASET", "dataset_code": "RECURRING"},
            {"code": "ECTM", "title": "Engine condition monitoring", "kind": "DATASET", "dataset_code": "ECTM"},
    ...
    {
        "code": "OPERATOR-RP",
        "name": "Operator-configurable Reliability Programme Report",
        "aircraft_family": "OPERATOR",
        "sections": [{"code": code.value, "title": definition.name, "kind": "DATASET", "dataset_code": code.value} for code, definition in DATASET_CATALOG.items()] + [{"code": "ALERTS", "title": "Statistical alert calculations", "kind": "STATISTICAL_ALERTS"}],
    },
]
...
def _normalise_payload(dataset: DatasetDefinition, payload: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if dataset.code == WorkbookDatasetCode.RM:
        if normalised.get("removal_type") == "UNSCHEDULED" and not normalised.get("reason_code"):
...
def _record_number(db: Session, amo_id: str, dataset_code: str, event_date: date) -> str:
    ...
    count = db.query(func.count(ReliabilityWorkbookRecord.id)).filter(
        ReliabilityWorkbookRecord.amo_id == amo_id,
        ReliabilityWorkbookRecord.dataset_code == dataset_code,
        ReliabilityWorkbookRecord.record_number.like(f"{prefix}-%"),
    ).scalar() or 0
...
def _event_type_for(record: ReliabilityWorkbookRecord) -> reliability_models.ReliabilityEventTypeEnum | None:
    ...
    mapping = {
        WorkbookDatasetCode.AI: reliability_models.ReliabilityEventTypeEnum.SAFETY_EVENT,
        WorkbookDatasetCode.OOS: reliability_models.ReliabilityEventTypeEnum.OTHER,
        WorkbookDatasetCode.SM: reliability_models.ReliabilityEventTypeEnum.DEFECT,
        WorkbookDatasetCode.STRUCTURES: reliability_models.ReliabilityEventTypeEnum.DEFECT,
        WorkbookDatasetCode.RECURRING: reliability_models.ReliabilityEventTypeEnum.REPEAT_DEFECT,
        WorkbookDatasetCode.ECTM: reliability_models.ReliabilityEventTypeEnum.ECTM,
    }
...
def _approve_to_canonical(db: Session, record: ReliabilityWorkbookRecord, user_id: str | None) -> None:
    if record.dataset_code == WorkbookDatasetCode.AU.value:
        existing = db.query(reliability_models.AircraftUtilizationDaily).filter(
        ...
        values = {
            "flight_hours": float(Decimal(record.payload["flight_hours"])),
            "cycles": float(Decimal(str(record.payload["flight_cycles"]))),
        ...
        if existing:
            ...
            db.add(reliability_models.AircraftUtilizationDaily(
    ...
    if event_type is None:
        return
    existing_event = db.query(reliability_models.ReliabilityEvent).filter(
        reliability_models.ReliabilityEvent.amo_id == record.amo_id,
        reliability_models.ReliabilityEvent.source_system == f"WORKBOOK-{record.dataset_code}",
        reliability_models.ReliabilityEvent.source_record_id == str(record.id),
    ).one_or_none()
    ...
    repeat_key = record.payload.get("repeat_key")
    part_number = record.payload.get("off_part_number") or record.payload.get("component_part_number") or record.payload.get("part_number")
    serial_number = record.payload.get("off_serial_number") or record.payload.get("component_serial_number") or record.payload.get("serial_number")
    if existing_event:
        record.canonical_event_id = existing_event.id
        return
    event = reliability_models.ReliabilityEvent(
        amo_id=record.amo_id,
        aircraft_serial_number=record.aircraft_serial_number,
        engine_position=record.payload.get("engine_position"),
        event_type=event_type,
        severity=severity,
        ata_chapter=record.ata_chapter,
        reference_code=record.reference_code or record.record_number,
        source_system=f"WORKBOOK-{record.dataset_code}",
        source_record_id=str(record.id),
        source_payload_hash=record.source_hash,
    ...
    if record.dataset_code == WorkbookDatasetCode.ECTM.value:
        metrics = {
            ...
            if key in {"pressure_altitude_ft", "oat_c", "isa_dev_c", "power_reference", "itt_c", "ng_pct", "nh_pct", "np_rpm", "torque", "fuel_flow", "oil_pressure", "oil_temperature", "vibration"} and value not in (None, "")
        }
        metrics.update({
            key: value for key, value in record.payload.items()
            if key in {"oil_analysis_status", "oil_analysis_reference", "borescope_status", "borescope_reference", "trend_status", "analyst_comments", "oem_recommendation", "review_date", "reviewed_by", "action_required"} and value not in (None, "")
        })
        db.add(reliability_models.EngineFlightSnapshot(
            amo_id=record.amo_id,
            aircraft_serial_number=record.aircraft_serial_number,
            engine_position=record.payload["engine_position"],
            engine_serial_number=record.payload.get("engine_serial_number"),
            flight_date=record.event_date,
            flight_leg=record.payload.get("flight_leg") or record.record_number,
            phase=record.payload.get("phase"),
            metrics=metrics,
            data_source="WORKBOOK-ECTM",
            source_record_id=str(record.id),
        ))
...
def _load_stat_series(db: Session, amo_id: str, request: StatisticalAlertRequest) -> list[dict[str, Any]]:
    ...
    if request.source_kind.startswith("EVENT"):
        ...
        events = query.order_by(reliability_models.ReliabilityEvent.occurred_at.asc()).limit(50_000).all()
        for row in events:
            buckets[_bucket_start(row.occurred_at.date(), request.bucket)] += Decimal("1")
        if request.source_kind == "EVENT_RATE_PER_100_FH":
            ...
            for row in utilisation.all():
                hours[_bucket_start(row.date, request.bucket)] += Decimal(str(row.flight_hours or 0))
            all_keys = sorted(set(buckets) | set(hours))
            return [{"period": key.isoformat(), "numerator": float(buckets[key]), "denominator": float(hours[key]), "value": float((buckets[key] / hours[key] * Decimal("100"))) if hours[key] > 0 else None} for key in all_keys]
    else:
        ...
        for row in query.order_by(ReliabilityWorkbookRecord.event_date.asc()).limit(50_000).all():
            key = _bucket_start(row.event_date, request.bucket)
            if request.source_kind == "DATASET_COUNT":
                buckets[key] += Decimal("1")
            else:
                ...
                if raw not in (None, ""):
                    buckets[key] += _decimal(raw, request.metric_field, minimum=None)
    return [{"period": key.isoformat(), "value": float(value)} for key, value in sorted(buckets.items())]
...
def _seed_layouts(db: Session, amo_id: str, user_id: str | None) -> list[ReliabilityReportLayout]:
    ...
    for definition in DEFAULT_LAYOUTS:
        key = (definition["code"], 1)
        row = existing.get(key)
        if not row:
            row = ReliabilityReportLayout(
                amo_id=amo_id, code=definition["code"], name=definition["name"], aircraft_family=definition["aircraft_family"],
                revision=1, active=True, sections=definition["sections"],
                page_settings={"size": "A4", "orientation": "portrait", "margin_mm": 10}, created_by_user_id=user_id,
            )
            db.add(row)
        output.append(row)
    db.flush()
...
def _render_report(db: Session, amo_id: str, layout: ReliabilityReportLayout, request: ReportRenderRequest) -> tuple[dict[str, Any], str]:
    ...
    for definition in layout.sections:
        ...
        if kind == "DATASET":
            query = db.query(ReliabilityWorkbookRecord).filter(
                ReliabilityWorkbookRecord.amo_id == amo_id,
                ReliabilityWorkbookRecord.dataset_code == definition.get("dataset_code"),
                ReliabilityWorkbookRecord.status.in_([WorkbookRecordStatus.APPROVED.value, WorkbookRecordStatus.CLOSED.value]),
                ReliabilityWorkbookRecord.event_date >= request.period_start,
                ReliabilityWorkbookRecord.event_date <= request.period_end,
            )
            ...
            query = db.query(ReliabilityStatisticalAlertResult).filter(
                ReliabilityStatisticalAlertResult.amo_id == amo_id,
                ReliabilityStatisticalAlertResult.period_start <= request.period_end,
                ReliabilityStatisticalAlertResult.period_end >= request.period_start,
            )
            rows = query.order_by(ReliabilityStatisticalAlertResult.metric_code.asc()).all()
...
def register(router: APIRouter) -> None:
    ...
    def list_records(
        ...
        if dataset_code:
            query = query.filter(ReliabilityWorkbookRecord.dataset_code == dataset_code.value)
        if aircraft_serial_number:
            query = query.filter(ReliabilityWorkbookRecord.aircraft_serial_number == aircraft_serial_number)
        if status_filter:
            query = query.filter(ReliabilityWorkbookRecord.status == status_filter.upper())
        if period_start:
            query = query.filter(ReliabilityWorkbookRecord.event_date >= period_start)
        if period_end:
            query = query.filter(ReliabilityWorkbookRecord.event_date <= period_end)
        if q:
            pattern = f"%{q.strip()}%"
    ...
    def create_record(payload: WorkbookRecordCreate, current_user: account_models.User = Depends(get_current_active_user), db: Session = Depends(get_write_db)):
        ...
        record = ReliabilityWorkbookRecord(
            amo_id=amo_id,
            dataset_code=payload.dataset_code.value,
            record_number=_record_number(db, amo_id, payload.dataset_code.value, payload.event_date),
            revision=1,
            status=WorkbookRecordStatus.DRAFT.value,
    ...
    def approve_record(record_id: int, action: RecordAction, current_user: account_models.User = Depends(get_current_active_user), db: Session = Depends(get_write_db)):
        ...
        if record.status != WorkbookRecordStatus.DRAFT.value:
            raise HTTPException(status_code=409, detail="Only draft records can be approved.")
        record.derived_values = {**(record.derived_values or {}), "approval_note": action.note}
        record.status = WorkbookRecordStatus.APPROVED.value
        record.approved_at = _now()
        record.approved_by_user_id = current_user.id
        _approve_to_canonical(db, record, current_user.id)
        db.commit()
        db.refresh(record)
        return record
    ...
    def close_record(record_id: int, action: RecordAction, current_user: account_models.User = Depends(get_current_active_user), db: Session = Depends(get_write_db)):
        ...
        if not record:
            raise HTTPException(status_code=404, detail="Workbook record not found.")
        if record.status != WorkbookRecordStatus.APPROVED.value:
            raise HTTPException(status_code=409, detail="Only approved records can be closed.")
        record.status = WorkbookRecordStatus.CLOSED.value
        record.closed_at = _now()
        record.closed_by_user_id = current_user.id
        record.derived_values = {**(record.derived_values or {}), "closure_note": action.note}
        db.commit()
        db.refresh(record)
        return record
    ...
    def oos_metrics(period_start: date, period_end: date, aircraft_serial_number: str | None = None, current_user: account_models.User = Depends(get_current_active_user), db: Session = Depends(get_write_db)):
        query = db.query(ReliabilityWorkbookRecord).filter(
            ReliabilityWorkbookRecord.amo_id == _amo_id(current_user), ReliabilityWorkbookRecord.dataset_code == WorkbookDatasetCode.OOS.value,
            ReliabilityWorkbookRecord.status.in_([WorkbookRecordStatus.APPROVED.value, WorkbookRecordStatus.CLOSED.value]),
            ReliabilityWorkbookRecord.event_date >= period_start, ReliabilityWorkbookRecord.event_date <= period_end,
        )
        if aircraft_serial_number:
            query = query.filter(ReliabilityWorkbookRecord.aircraft_serial_number == aircraft_serial_number)
        rows = query.all()
        downtime = sum((Decimal(str(row.derived_values.get("downtime_hours") or 0)) for row in rows), Decimal("0"))
        scheduled = sum((Decimal(str(row.payload.get("scheduled_available_hours") or 0)) for row in rows), Decimal("0"))
        completed = [Decimal(str(row.derived_values.get("downtime_hours"))) for row in rows if row.derived_values.get("downtime_hours") not in (None, "")]
        available = max(scheduled - downtime, Decimal("0"))
    ...
    @router.post("/workbook-parity/statistical-alerts", status_code=201)
    def calculate_statistical_alert(request: StatisticalAlertRequest, current_user: account_models.User = Depends(get_current_active_user), db: Session = Depends(get_write_db)):
        amo_id = _amo_id(current_user)
        series = _load_stat_series(db, amo_id, request)
        values = [float(row["value"]) for row in series if row.get("value") is not None and math.isfinite(float(row["value"]))]
        if len(values) < 2:
            raise HTTPException(status_code=422, detail="At least two populated analytical periods are required to calculate a sample standard deviation.")
        mean_value = Decimal(str(statistics.fmean(values)))
        stddev = Decimal(str(statistics.stdev(values)))
        warning = mean_value + request.warning_multiplier * stddev
        alert = mean_value + request.alert_multiplier * stddev
        formula = f"warning = mean + {request.warning_multiplier} × sample standard deviation; alert = mean + {request.alert_multiplier} × sample standard deviation"
        result = ReliabilityStatisticalAlertResult(
            ...
            scope_value=request.aircraft_serial_number or request.ata_chapter,
            period_start=request.period_start, period_end=request.period_end, bucket=request.bucket, sample_size=len(values),
            mean_value=mean_value, sample_stddev=stddev, warning_multiplier=request.warning_multiplier, alert_multiplier=request.alert_multiplier,
            warning_level=warning, alert_level=alert, formula=formula, series=series, generated_by_user_id=current_user.id,
        )
        db.add(result)
        db.commit()
        db.refresh(result)
        return {"id": result.id, "metric_code": result.metric_code, "metric_label": result.metric_label, "sample_size": result.sample_size, "mean": float(result.mean_value), "sample_stddev": float(result.sample_stddev), "warning_level": float(result.warning_level), "alert_level": float(result.alert_level), "formula": result.formula, "series": result.series, "generated_at": result.generated_at}
    ...
    def create_mapping(payload: MappingCreate, current_user: account_models.User = Depends(get_current_active_user), db: Session = Depends(get_write_db)):
        definition = DATASET_CATALOG[payload.dataset_code]
        valid_fields = {field.key for field in definition.fields} | {"event_date", "event_end_date", "aircraft_serial_number", "ata_chapter", "reference_code", "title", "description"}
        if payload.canonical_field not in valid_fields:
            raise HTTPException(status_code=422, detail="Canonical field is not defined for the selected dataset.")
        row = ReliabilityWorkbookFieldMapping(amo_id=_amo_id(current_user), created_by_user_id=current_user.id, **payload.model_dump(mode="json"))
        db.add(row)
        db.commit()
        db.refresh(row)
    ...
    def list_mappings(profile_code: str | None = None, current_user: account_models.User = Depends(get_current_active_user), db: Session = Depends(get_write_db)):
        ...
        if profile_code:
            query = query.filter(ReliabilityWorkbookFieldMapping.profile_code == profile_code)
        rows = query.order_by(ReliabilityWorkbookFieldMapping.workbook_family, ReliabilityWorkbookFieldMapping.dataset_code, ReliabilityWorkbookFieldMapping.source_sheet, ReliabilityWorkbookFieldMapping.id).all()
    ...
    def parity(current_user: account_models.User = Depends(get_current_active_user), db: Session = Depends(get_write_db)):
        amo_id = _amo_id(current_user)
        mappings = db.query(ReliabilityWorkbookFieldMapping).filter(ReliabilityWorkbookFieldMapping.amo_id == amo_id, ReliabilityWorkbookFieldMapping.active.is_(True)).all()
        ...
        for code, definition in DATASET_CATALOG.items():
            ...
            optional = {field.key for field in definition.fields if not field.required}
    ...
    def seed_report_layouts(current_user: account_models.User = Depends(get_current_active_user), db: Session = Depends(get_write_db)):
        rows = _seed_layouts(db, _amo_id(current_user), current_user.id)
        db.commit()
        return [{"id": row.id, "code": row.code, "name": row.name, "aircraft_family": row.aircraft_family, "revision": row.revision, "active": row.active, "sections": row.sections, "page_settings": row.page_settings} for row in rows]
    ...
    def create_report_layout(payload: ReportLayoutCreate, current_user: account_models.User = Depends(get_current_active_user), db: Session = Depends(get_write_db)):
        amo_id = _amo_id(current_user)
        latest = db.query(func.max(ReliabilityReportLayout.revision)).filter(ReliabilityReportLayout.amo_id == amo_id, ReliabilityReportLayout.code == payload.code).scalar() or 0
        db.query(ReliabilityReportLayout).filter(ReliabilityReportLayout.amo_id == amo_id, ReliabilityReportLayout.code == payload.code, ReliabilityReportLayout.active.is_(True)).update({"active": False}, synchronize_session=False)
        row = ReliabilityReportLayout(amo_id=amo_id, revision=int(latest) + 1, active=True, created_by_user_id=current_user.id, **payload.model_dump(mode="json"))
        db.add(row)
        db.commit()
        db.refresh(row)
        return {"id": row.id, "code": row.code, "name": row.name, "aircraft_family": row.aircraft_family, "revision": row.revision, "active": row.active, "sections": row.sections, "page_settings": row.page_settings}
    ...
    def list_report_layouts(current_user: account_models.User = Depends(get_current_active_user), db: Session = Depends(get_write_db)):
        amo_id = _amo_id(current_user)
        if not db.query(ReliabilityReportLayout.id).filter(ReliabilityReportLayout.amo_id == amo_id).first():
            _seed_layouts(db, amo_id, current_user.id)
            db.commit()
        rows = db.query(ReliabilityReportLayout).filter(ReliabilityReportLayout.amo_id == amo_id).order_by(ReliabilityReportLayout.code, ReliabilityReportLayout.revision.desc()).all()
        return [{"id": row.id, "code": row.code, "name": row.name, "aircraft_family": row.aircraft_family, "revision": row.revision, "active": row.active, "sections": row.sections, "page_settings": row.page_settings} for row in rows]
    ...
    def render_report(payload: ReportRenderRequest, current_user: account_models.User = Depends(get_current_active_user), db: Session = Depends(get_write_db)):
        amo_id = _amo_id(current_user)
        layout = db.query(ReliabilityReportLayout).filter(ReliabilityReportLayout.id == payload.layout_id, ReliabilityReportLayout.amo_id == amo_id).one_or_none()
        if not layout:
            raise HTTPException(status_code=404, detail="Reliability report layout not found.")
        data, rendered = _render_report(db, amo_id, layout, payload)
        checksum = hashlib.sha256(rendered.encode()).hexdigest()
        snapshot = ReliabilityWorkbookReportSnapshot(amo_id=amo_id, layout_id=layout.id, period_start=payload.period_start, period_end=payload.period_end, aircraft_filter=payload.aircraft, rendered_data=data, rendered_html=rendered, sha256_hash=checksum, generated_by_user_id=current_user.id)
        db.add(snapshot)
        db.commit()
        db.refresh(snapshot)
