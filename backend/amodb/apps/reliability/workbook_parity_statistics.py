...
from .workbook_parity import (
    ReliabilityStatisticalAlertResult,
    ReliabilityWorkbookRecord,
...
def complete_bucket_sequence(period_start: date, period_end: date, bucket: str) -> list[date]:
    current = bucket_start(period_start, bucket)
    end = bucket_start(period_end, bucket)
    values: list[date] = []
    while current <= end:
...
def load_complete_series(db: Session, amo_id: str, request: StatisticalAlertRequest) -> list[dict[str, Any]]:
    ...
    if request.source_kind.startswith("EVENT"):
        ...
        events = query.order_by(reliability_models.ReliabilityEvent.occurred_at.asc()).limit(MAX_SCAN + 1).all()
        ...
        utilisation_rows = utilisation.order_by(reliability_models.AircraftUtilizationDaily.date.asc()).limit(MAX_SCAN + 1).all()
        if len(utilisation_rows) > MAX_SCAN:
        ...
        return [
            {
                ...
                "numerator": float(values[key]),
                "denominator": float(hours[key]),
                "value": float(values[key] / hours[key] * Decimal("100")) if hours[key] > 0 else None,
            }
            for key in sequence
        ]
...
def calculate_levels(series: list[dict[str, Any]], warning_multiplier: Decimal, alert_multiplier: Decimal) -> dict[str, Any]:
    ...
    warning = mean_value + warning_multiplier * stddev
    alert = mean_value + alert_multiplier * stddev
    return {
...
def register(router: APIRouter) -> None:
    ...
    def calculate_statistical_alert(
        request: StatisticalAlertRequest,
        ...
        formula = (
            f"warning = arithmetic mean + {request.warning_multiplier} × sample standard deviation; "
            f"alert = arithmetic mean + {request.alert_multiplier} × sample standard deviation"
        )
        result = ReliabilityStatisticalAlertResult(
            ...
            dataset_code=request.dataset_code.value if request.dataset_code else None,
            ...
            sample_size=levels["sample_size"],
            mean_value=levels["mean"],
            sample_stddev=levels["sample_stddev"],
            warning_multiplier=request.warning_multiplier,
            alert_multiplier=request.alert_multiplier,
            warning_level=levels["warning_level"],
        ...
        return {
            ...
            "sample_stddev": float(result.sample_stddev),
            "warning_level": float(result.warning_level),
            "alert_level": float(result.alert_level),
