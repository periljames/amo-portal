...
from .workbook_parity import (
    DATASET_CATALOG,
    DEFAULT_LAYOUTS,
    ReliabilityWorkbookFieldMapping,
    WorkbookDatasetCode,
)
...
WORKBOOK_PROFILES: tuple[dict[str, str], ...] = (
    {
        "code": "SAFARILINK-C208B-RP",
        "name": "Safarilink Cessna 208B Reliability Programme",
        "family": "C208B",
    },
    {
        "code": "SAFARILINK-DHC8-RP",
        "name": "Safarilink DHC8 Reliability Programme",
        "family": "DHC8",
    },
    {
        "code": "GENERIC-ANALYSIS-TEMPLATE",
        "name": "Generic Reliability Analysis Template",
        "family": "OPERATOR",
    },
)

# Workbook terminology that differs from the normalized portal labels. The
# canonical label is also always retained as an alias.
                ...
                rows.append(
                    {
                        ...
                        "workbook_family": profile["family"],
                        "dataset_code": dataset_code.value,
                        "source_sheet": sheet,
                        "source_column": field.label,
                        ...
                        "aliases": aliases,
                        "transform": {
                            "trim": field.data_type in {"text", "textarea", "select"},
                            "uppercase": field.data_type == "select",
                            "exact_numeric": field.data_type in {"decimal", "integer"},
                        },
                    }
                )
    return rows
...
def mapping_contract() -> dict[str, Any]:
    expected_by_dataset = {
        code.value: {field.key for field in definition.fields}
        for code, definition in DATASET_CATALOG.items()
    }
    required_by_dataset = {
        code.value: {field.key for field in definition.fields if field.required}
        for code, definition in DATASET_CATALOG.items()
    }
    result: dict[str, Any] = {"profiles": {}, "datasets": {}}
    for profile in WORKBOOK_PROFILES:
        profile_rows = [row for row in DEFAULT_MAPPING_ROWS if row["profile_code"] == profile["code"]]
        mapped: dict[str, set[str]] = defaultdict(set)
        for row in profile_rows:
            mapped[row["dataset_code"]].add(row["canonical_field"])
        profile_result = {}
        for dataset_code, expected in expected_by_dataset.items():
            missing = sorted(expected - mapped[dataset_code])
            missing_required = sorted(required_by_dataset[dataset_code] - mapped[dataset_code])
            profile_result[dataset_code] = {
                "mapped_fields": len(mapped[dataset_code]),
                "expected_fields": len(expected),
                "missing_fields": missing,
                "missing_required_fields": missing_required,
                "coverage_pct": round(len(mapped[dataset_code] & expected) / max(len(expected), 1) * 100, 1),
            }
        result["profiles"][profile["code"]] = profile_result
    for code, definition in DATASET_CATALOG.items():
        result["datasets"][code.value] = {
            "name": definition.name,
            "sheet_names": definition.workbook_sheet_names,
            "field_count": len(definition.fields),
            "required_count": sum(field.required for field in definition.fields),
        }
    return result


def layout_contract() -> dict[str, Any]:
    required_datasets = {code.value for code in WorkbookDatasetCode}
    layouts = {}
    for layout in DEFAULT_LAYOUTS:
        datasets = {
            str(section.get("dataset_code"))
            for section in layout["sections"]
            if section.get("kind") == "DATASET"
        }
        layouts[layout["code"]] = {
            "datasets": sorted(datasets),
            "missing_datasets": sorted(required_datasets - datasets),
            "has_statistical_alerts": any(section.get("kind") == "STATISTICAL_ALERTS" for section in layout["sections"]),
        }
    return {"required_datasets": sorted(required_datasets), "layouts": layouts}
...
def register(router: APIRouter) -> None:
    ...
    def seed_default_mappings(
        ...
        existing = {
            (row.profile_code, row.dataset_code, row.source_sheet, row.source_column): row
            for row in db.query(ReliabilityWorkbookFieldMapping).filter(
                ReliabilityWorkbookFieldMapping.amo_id == amo_id
            ).all()
        }
        created = 0
        repaired = 0
        ...
        db.commit()
        return {
            "profiles": [profile["code"] for profile in WORKBOOK_PROFILES],
            "expected_rows": len(DEFAULT_MAPPING_ROWS),
            "created": created,
            "repaired": repaired,
            "total_active": db.query(func.count(ReliabilityWorkbookFieldMapping.id)).filter(
                ReliabilityWorkbookFieldMapping.amo_id == amo_id,
                ReliabilityWorkbookFieldMapping.active.is_(True),
            ).scalar() or 0,
    ...
    def read_parity_contracts(current_user: account_models.User = Depends(get_current_active_user)):
        ...
        return {
            ...
            "report_layouts": layout_contract(),
        }
