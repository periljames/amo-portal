...
def test_oos_derives_downtime_availability_and_validates_chronology():
    ...
    normalised, derived = workbook_parity._normalise_payload(
        definition,
        {
            "start_at": "2026-08-01T06:00:00+00:00",
            "end_at": "2026-08-01T12:00:00+00:00",
            "reason_category": "TECHNICAL",
            "maintenance_action": "Replaced failed starter-generator.",
            "scheduled_available_hours": "24.000",
        },
    )
    assert normalised["reason_category"] == "TECHNICAL"
    assert derived["downtime_hours"] == "6.000"
    assert derived["available_hours"] == "18.000"
    assert derived["availability_pct"] == "75.000"

    with pytest.raises(HTTPException, match="cannot precede"):
        workbook_parity._normalise_payload(
            definition,
            {
                "start_at": "2026-08-02T12:00:00+00:00",
                "end_at": "2026-08-02T06:00:00+00:00",
                "reason_category": "TECHNICAL",
                "maintenance_action": "Invalid chronology.",
            },
        )
...
def test_recurring_defect_requires_actual_recurrence_and_ordered_dates():
    ...
    base = {
        "repeat_key": "ATA21-PACK-VALVE",
        "first_seen": "2026-01-01",
        "last_seen": "2026-02-01",
        "origin": "PIREP",
        "defect_description": "Pack valve failed to regulate.",
        "occurrence_count": 2,
        "recurrence_window_days": 31,
    ...
    with pytest.raises(HTTPException, match="cannot precede"):
        workbook_parity._normalise_payload(
            definition,
            {**base, "first_seen": "2026-03-01", "last_seen": "2026-02-01"},
        )
...
def test_unscheduled_removal_requires_failure_reason_and_exact_life_values():
    ...
    payload = {
        "removed_at": "2026-08-01T08:00:00+00:00",
        "component_description": "Starter-generator",
        "off_part_number": "23085-001",
        "off_serial_number": "SG-1001",
        "removal_type": "UNSCHEDULED",
        "reason_code": "NO OUTPUT",
        "hours_at_removal": "1245.375",
        "cycles_at_removal": 1811,
    }
...
def test_default_mapping_rows_are_unique_per_profile_sheet_and_column():
    keys = [
        ...
        for row in workbook_parity_defaults.DEFAULT_MAPPING_ROWS
    ]
    assert len(keys) == len(set(keys))
    assert all(row["aliases"] for row in workbook_parity_defaults.DEFAULT_MAPPING_ROWS)


def test_report_layouts_cover_required_operator_output_and_statistical_alerts():
    ...
    c208 = contract["layouts"]["C208B-RP"]
    assert {"AU", "PM", "RM", "OOS", "RECURRING", "ECTM"} <= set(c208["datasets"])
    assert c208["has_statistical_alerts"] is True

    dhc8 = contract["layouts"]["DHC8-RP"]
    assert {"AU", "AI", "SM", "STRUCTURES", "RM", "OOS", "RECURRING", "ECTM"} <= set(dhc8["datasets"])
    assert dhc8["has_statistical_alerts"] is True


def test_statistical_alert_contract_uses_sample_standard_deviation():
    source = inspect.getsource(workbook_parity.register)
    assert "statistics.stdev(values)" in source
    assert "mean + {request.warning_multiplier} × sample standard deviation" in source
    request = workbook_parity.StatisticalAlertRequest(
        metric_code="PIREPS_PER_100_FH",
        metric_label="Pilot reports per 100 flight hours",
        source_kind="EVENT_RATE_PER_100_FH",
        period_start=date(2025, 1, 1),
        period_end=date(2026, 1, 1),
        event_types=["PILOT_REPORT"],
        warning_multiplier="1",
        alert_multiplier="2",
    )
    assert request.alert_multiplier == 2
    assert request.warning_multiplier == 1
...
def test_router_registers_every_workbook_parity_surface():
    ...
    expected = {
        "/reliability/workbook-parity/catalog",
        "/reliability/workbook-parity/records",
        "/reliability/workbook-parity/records/{record_id}/approve",
        "/reliability/workbook-parity/records/{record_id}/close",
        "/reliability/workbook-parity/oos-metrics",
        "/reliability/workbook-parity/statistical-alerts",
        "/reliability/workbook-parity/mappings",
        "/reliability/workbook-parity/mappings/seed-defaults",
        "/reliability/workbook-parity/parity",
        "/reliability/workbook-parity/contracts",
        "/reliability/workbook-parity/report-layouts",
        "/reliability/workbook-parity/report-layouts/seed",
        "/reliability/workbook-parity/reports/render",
        "/reliability/workbook-parity/reports",
        "/reliability/workbook-parity/reports/{report_id}/html",
    }
