...
router = APIRouter(prefix="/accounts/admin-profile", tags=["admin_profile"])
...
def _eligible_grant(db: Session, *, amo_id: str, user_id: str, now: datetime) -> dict | None:
    row = db.execute(
        text("""
...
def _active_session(db: Session, *, amo_id: str, user_id: str, now: datetime) -> dict | None:
    row = db.execute(
        text("""
...
def _state(db: Session, *, amo: models.AMO, user: models.User) -> dict:
    _ensure_schema(db)
    now = _utcnow()
    db.execute(
        text("UPDATE admin_profile_sessions SET revoked_at = :now WHERE amo_id = :amo_id AND user_id = :user_id AND revoked_at IS NULL AND expires_at <= :now"),
...
def _require_active_profile(db: Session, *, amo: models.AMO, user: models.User) -> dict:
    state = _state(db, amo=amo, user=user)
    if not state["active"]:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Activate Admin profile before performing this administration action.")
    return state
...
def activate_admin_profile(
    ...
    db.execute(
        text("UPDATE admin_profile_sessions SET revoked_at = :now WHERE amo_id = :amo_id AND user_id = :user_id AND revoked_at IS NULL"),
        {"now": now, "amo_id": str(amo.id), "user_id": str(current_user.id)},
    )
    expires_at = now + timedelta(minutes=SESSION_DURATION_MINUTES)
    if grant and grant.get("valid_until") and grant["valid_until"] < expires_at:
        expires_at = grant["valid_until"]
    db.execute(
        text("""
            INSERT INTO admin_profile_sessions (id, amo_id, user_id, grant_id, activated_at, expires_at, revoked_at, created_at)
            VALUES (:id, :amo_id, :user_id, :grant_id, :activated_at, :expires_at, NULL, :created_at)
        """),
        {
            "id": str(uuid4()),
            "amo_id": str(amo.id),
...
def list_admin_grants(
    ...
    rows = db.execute(
        text("""
            SELECT g.*,
                   (SELECT COUNT(DISTINCT a.approver_user_id) FROM admin_access_grant_approvals a WHERE a.grant_id = g.id AND a.decision = 'APPROVED') AS approval_count
...
def request_admin_grant(
    ...
    _assert_tenant_member(current_user, amo)
    _require_active_profile(db, amo=amo, user=current_user)
    target = db.query(models.User).filter(models.User.id == payload.user_id, models.User.amo_id == amo.id).first()
    if not target or not target.is_active:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Active tenant user was not found.")
    if payload.grant_type == "TEMPORARY" and not payload.valid_until:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Temporary administrator grants require an expiry time.")
    if payload.valid_until and payload.valid_until <= (payload.valid_from or _utcnow()):
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Grant expiry must be later than its start time.")
    now = _utcnow()
    grant_id = str(uuid4())
    db.execute(
        text("""
            INSERT INTO admin_access_grants (
                id, amo_id, user_id, grant_type, valid_from, valid_until, status, reason,
                requested_by_user_id, activated_at, revoked_at, revoked_by_user_id, created_at, updated_at
            ) VALUES (
                :id, :amo_id, :user_id, :grant_type, :valid_from, :valid_until, 'PENDING', :reason,
                :requested_by_user_id, NULL, NULL, NULL, :created_at, :updated_at
            )
        """),
        {
            "id": grant_id,
    ...
    db.commit()
    return {"id": grant_id, "status": "PENDING", "approval_count": 0, "required_approvals": 2}
...
def approve_admin_grant(
    ...
    try:
        db.execute(
            text("""
                INSERT INTO admin_access_grant_approvals (id, grant_id, approver_user_id, decision, comment, created_at)
                VALUES (:id, :grant_id, :approver_user_id, 'APPROVED', :comment, :created_at)
            """),
            {"id": str(uuid4()), "grant_id": grant_id, "approver_user_id": str(current_user.id), "comment": payload.comment, "created_at": now},
        )
        db.flush()
    except Exception as exc:
        db.rollback()
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="This administrator has already approved the grant.") from exc
    count = _approval_count(db, grant_id)
    if count >= 2:
        db.execute(
            text("UPDATE admin_access_grants SET status = 'ACTIVE', activated_at = :now, updated_at = :now WHERE id = :grant_id"),
            {"now": now, "grant_id": grant_id},
        )
    db.commit()
    return {"id": grant_id, "status": "ACTIVE" if count >= 2 else "PENDING", "approval_count": count, "required_approvals": 2}
...
def revoke_admin_grant(
    ...
    now = _utcnow()
    db.execute(
        text("UPDATE admin_access_grants SET status = 'REVOKED', revoked_at = :now, revoked_by_user_id = :actor, updated_at = :now WHERE id = :grant_id"),
    ...
    db.commit()
    return {"id": grant_id, "status": "REVOKED", "comment": payload.comment}
