...
def _serialize(record: document_models.ProcurementDocument, amo_code: str) -> document_schemas.ProcurementDocumentRead:
    return document_schemas.ProcurementDocumentRead(
        ...
        download_url=f"/api/maintenance/{amo_code}/procurement/documents/{record.id}/download",
    )
...
@router.post(
    "/documents",
    response_model=document_schemas.ProcurementDocumentRead,
    status_code=status.HTTP_201_CREATED,
)
def procurement_document_upload(
    amo_code: str,
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="The document metadata could not be committed.") from exc
...
def procurement_document_download(
    ...
    return FileResponse(
        ...
        headers={
            ...
            "X-Procurement-Document-SHA256": record.sha256,
        },
    )
...
def procurement_document_void(
    ...
    record = document_service.void_document(
        ...
        actor_user_id=current_user.id,
        actor_is_quality=current_user.role in {
            account_models.AccountRole.QUALITY_MANAGER,
            account_models.AccountRole.QUALITY_INSPECTOR,
            account_models.AccountRole.AMO_ADMIN,
