...
TARGET_REVISION = "quality_260808_prog_schedule"
...
def _bootstrap(engine: sa.Engine) -> dict[str, str]:
    ...
    with engine.begin() as connection:
        ...
        # The programme lineage migration references the existing authoritative
        # planner schedule table. The probe stamps the prior chain, so create the
        # minimum predecessor table shape required to validate that FK.
...
def main() -> None:
    ...
    with engine.begin() as connection:
        ...
        item_columns = {
            row.column_name
            for row in connection.execute(text("""SELECT column_name FROM information_schema.columns
                WHERE table_schema=current_schema() AND table_name='quality_audit_programme_items'"""))
        }
        assert {"schedule_id", "scheduled_by_user_id", "scheduled_at"}.issubset(item_columns)
        schedule_fk = connection.execute(text("""SELECT COUNT(*) FROM information_schema.table_constraints tc
            JOIN information_schema.constraint_column_usage ccu ON ccu.constraint_name=tc.constraint_name AND ccu.constraint_schema=tc.constraint_schema
            WHERE tc.table_schema=current_schema() AND tc.table_name='quality_audit_programme_items'
              AND tc.constraint_type='FOREIGN KEY' AND ccu.table_name='qms_audit_schedules' AND ccu.column_name='id'""")).scalar_one()
        assert schedule_fk == 1
    ...
    programme = str(uuid4()); universe = str(uuid4()); item = str(uuid4()); event = str(uuid4()); schedule = str(uuid4())
    with engine.begin() as connection:
        connection.execute(text("INSERT INTO qms_audit_schedules (id) VALUES (:id)"), {"id": schedule})
        ...
        connection.execute(text("""INSERT INTO quality_audit_programmes
            ...
            VALUES (:id,:amo,'AP-2026-PROBE-R01','AP-2026-PROBE',2026,1,'2026 Audit Programme',CAST('[]' AS json),CAST('[]' AS json),'APPROVED','2026-01-01','2026-12-31',:user,:user,:user,NOW(),NOW())"""),
        ...
        connection.execute(text("""INSERT INTO quality_audit_programme_items
            (id,amo_id,programme_id,universe_item_id,schedule_id,audit_type,title,scope,criteria,mandatory_surveillance,recurrence,state,prioritization_basis,scheduled_by_user_id,scheduled_at,created_by_user_id,updated_by_user_id,created_at,updated_at)
            VALUES (:id,:amo,:programme,:universe,:schedule,'DEPARTMENTAL','Maintenance Department Audit','Departmental quality system',CAST('[]' AS json),true,'ANNUAL','SCHEDULED',CAST('[]' AS json),:user,NOW(),:user,:user,NOW(),NOW())"""),
            {"id": item, "amo": ids["amo_a"], "programme": programme, "universe": universe, "schedule": schedule, "user": ids["user_a"]})
        connection.execute(text("""INSERT INTO quality_audit_programme_events
            ...
            VALUES (:id,:amo,:programme,'ITEM_SCHEDULED','Requirement linked to authoritative planner schedule',CAST('{}' AS json),:user,NOW())"""),
        ...
        linked = connection.execute(text("SELECT schedule_id::text,state,scheduled_by_user_id FROM quality_audit_programme_items WHERE id=:id"), {"id": item}).one()
        assert linked.schedule_id == schedule
        assert linked.state == "SCHEDULED"
        assert linked.scheduled_by_user_id == ids["user_a"]

    with engine.begin() as connection:
        connection.execute(text(f"SET LOCAL ROLE {APP_ROLE}")); _set_tenant(connection, ids["amo_b"], ids["user_b"])
        for table in tables:
            assert connection.execute(text(f"SELECT COUNT(*) FROM {table}")).scalar_one() == 0
        assert connection.execute(text("UPDATE quality_audit_programme_items SET state='COMPLETED' WHERE id=:id"), {"id": item}).rowcount == 0
        assert connection.execute(text("DELETE FROM quality_audit_programme_events WHERE id=:id"), {"id": event}).rowcount == 0

    print("Quality Audit Programme migration, planner lineage and tenant isolation probe passed")
