...
def _is_command_search_route(route_item) -> bool:
    return "/quality/command-search" in str(getattr(route_item, "path", ""))
...
def _promote(api_router: APIRouter) -> None:
    routes = [item for item in api_router.routes if _is_command_search_route(item)]
    if not routes:
        raise RuntimeError("QMS command-search routes were not registered")
    remaining = [item for item in api_router.routes if not _is_command_search_route(item)]
