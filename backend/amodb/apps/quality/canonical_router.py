...
def _valid_zoneinfo(value: str | None):
    """Resolve IANA/UTC timezone labels with deterministic fallback support.

    Returns ``(tzinfo, label, warning)``. ``UTC±HH:MM`` fixed offsets are
    accepted even when OS tzdata is unavailable.
    """
    label = (value or "UTC").strip() or "UTC"
    if label.upper() == "UTC":
        return timezone.utc, "UTC", None
    try:
        return ZoneInfo(label), label, None
        ...
        if fixed:
            ...
            if hours <= 14 and minutes < 60:
                ...
                if sign == "-":
                    offset = -offset
                return timezone(offset), f"UTC{sign}{hours:02d}:{minutes:02d}", None
        return timezone.utc, "UTC", f"Timezone '{label}' was not available; UTC used."
...
def _pg_set_read_timeout(db: Session, timeout_ms: int = 5000) -> None:
    """Set a bounded timeout for the current PostgreSQL transaction/read."""
    if _is_postgres(db):
        db.execute(text(f"SET LOCAL statement_timeout = '{int(timeout_ms)}ms'"))


def _recover_qms_read_session(db: Session, *, amo_id: str, user_id: str, timeout_ms: int = 5000) -> None:
    """Recover a read session after PostgreSQL cancels a statement."""
    try:
        db.rollback()
    except Exception:
        pass
    try:
        ...
        # The original error should stay visible to the caller.
        pass


def _safe_qms_scalar(
    db: Session,
    *,
    sql: str,
    params: dict[str, Any],
    ...
    try:
        ...
        logger.error("Quality dashboard counter failed trace_id=%s label=%s error=%s", trace_id, label, exc)
        source_errors.append({"label": label, "message": str(exc), "type": exc.__class__.__name__})
        _recover_qms_read_session(db, amo_id=amo_id, user_id=user_id, timeout_ms=timeout_ms)
        return None


def _event_with_state(module: str, entity_type: str, entity_id: str, title: str, when: Any, event_type: str, link: str | None = None, *, today: date | None = None, actionable: bool = True) -> dict[str, Any]:
    item = _event(module, entity_type, entity_id, title, when, event_type, link)
    event_date = when if isinstance(when, date) and not isinstance(when, datetime) else (when.date() if isinstance(when, datetime) else None)
    if today and event_date:
        item["due_state"] = "overdue" if actionable and event_date < today else "today" if event_date == today else "upcoming"
    else:
        item["due_state"] = "scheduled"
...
def qms_dashboard(
    ...
    _pg_set_read_timeout(db, 5000)
    today = date.today()
    due_soon = today + timedelta(days=30)
    cap = 5000
    source_errors: list[dict[str, Any]] = []
    counter_precision: dict[str, str] = {}

    def capped(label: str, sql: str, extra: dict[str, Any] | None = None) -> int:
        params = {"amo_id": ctx.amo_id, "today": today, "due_soon": due_soon, "cap": cap + 1}
        if extra:
            params.update(extra)
        value = _safe_qms_scalar(
            db,
            sql=sql,
            params=params,
            trace_id=trace_id,
        ...
        if value is None:
            counter_precision[label] = "error"
            return 0
        if value > cap:
            counter_precision[label] = "at_least"
            return cap
        counter_precision[label] = "exact"
        return value

    counters = {
        # The dashboard is an operational planning surface.  The historical audit register
        # can be large in production, so live dashboard indicators use the indexed
        # audit schedule/planning table instead of scanning qms_audits on every page
        # load.  Audit register/detail pages still read qms_audits directly.
        "open_audits": capped(
            "open_audits",
            """
            SELECT COUNT(*) FROM (
                SELECT 1 FROM qms_audit_schedules
                WHERE amo_id = :amo_id
        ...
        "audits_due_soon": capped(
            ...
            SELECT COUNT(*) FROM (
                ...
                WHERE amo_id = :amo_id
                  ...
                  AND next_due_date <= :due_soon
                LIMIT :cap
            ) s
            """,
        ),
        "active_audit_fieldwork": capped(
            "active_audit_fieldwork",
            """
            SELECT COUNT(*) FROM (
                SELECT 1 FROM qms_audit_schedules
                WHERE amo_id = :amo_id
        ...
        "cars_due_soon": capped(
            ...
            SELECT COUNT(*) FROM (
                ...
                WHERE amo_id = :amo_id
                  AND due_date IS NOT NULL
                  AND due_date >= :today
                  AND due_date <= :due_soon
                  AND status NOT IN ('CLOSED', 'CANCELLED')
                LIMIT :cap
            ) s
            """,
        ),
        "open_findings": capped(
            "open_findings",
            """
            SELECT COUNT(*) FROM (
                SELECT 1 FROM qms_audit_findings
                WHERE amo_id = :amo_id AND closed_at IS NULL
                LIMIT :cap
            ) s
            """,
        ),
        "draft_documents": capped(
            "draft_documents",
            """
            SELECT COUNT(*) FROM (
                SELECT 1 FROM qms_documents
                WHERE amo_id = :amo_id AND status = 'DRAFT'
                LIMIT :cap
            ) s
            """,
        ),
        "active_documents": capped(
            "active_documents",
            """
            SELECT COUNT(*) FROM (
                SELECT 1 FROM qms_documents
        ...
        "training_expired_records": capped(
            "training_expired_records",
            """
            SELECT COUNT(*) FROM (
    ...
    return {
        ...
        "source_errors": source_errors,
        "warning": "Some QMS counters failed. See source_errors." if source_errors else None,
        "links": {
            "open_cars": f"/maintenance/{ctx.amo_code}/quality/cars/overdue",
            "audits_due_soon": f"/maintenance/{ctx.amo_code}/quality/audits/schedule",
            "documents": f"/maintenance/{ctx.amo_code}/quality/documents/library",
            "training": f"/maintenance/{ctx.amo_code}/training/competence/overdue",
        },
        "trace_id": trace_id,
...
@core_router.get("/inbox")
def qms_inbox(
    status_filter: str | None = Query(None, alias="status"),
    ctx: TenantContext = Depends(require_quality_permission("qms.inbox.view")),
    db: Session = Depends(get_read_db),
) -> dict[str, Any]:
    set_postgres_tenant_context(db, amo_id=ctx.amo_id, user_id=ctx.user_id)
    qs = (
...
def _calendar_source_audits(
    ...
    *,
    ...
    """Return audit calendar items from the audit schedule source.

    The operational calendar is a planning surface, so it should read the indexed
    qms_audit_schedules.next_due_date records first. The live qms_audits table can
    be very large in deployed tenants and is used by audit registers/detail views,
    not as the primary month-board source.
    """
    try:
        _pg_set_read_timeout(db, 5000)
        rows = db.execute(text("""
            SELECT id, title, kind, frequency, next_due_date AS event_date, is_active
            ...
            LIMIT :limit
        """), {"amo_id": ctx.amo_id, "start_date": start_date, "end_date": end_date, "limit": source_limit}).mappings().all()
    except Exception as exc:
        logger.error("QMS calendar source failed trace_id=%s label=audit_schedule error=%s", trace_id, exc)
        source_errors.append({"label": "audit_schedule", "message": str(exc), "type": exc.__class__.__name__})
        _recover_qms_read_session(db, amo_id=ctx.amo_id, user_id=ctx.user_id, timeout_ms=5000)
        return []

    events: list[dict[str, Any]] = []
    for row in rows:
        title = str(row.get("title") or "Scheduled audit")
        kind = str(row.get("kind") or "audit").replace("_", " ").title()
        label = f"{kind}: {title}"
        events.append(_event_with_state(
            "audits",
            "audit_schedule",
            str(row["id"]),
            label,
            row["event_date"],
            "audit_due",
            f"/maintenance/{ctx.amo_code}/quality/audits/schedule",
            today=today,
        ))
    return events

def _calendar_source_cars(
    db: Session,
    *,
    ctx: TenantContext,
    start_date: date,
    ...
    try:
        _pg_set_read_timeout(db, 5000)
        rows = db.execute(text("""
            SELECT id, car_number, title, status, due_date
            FROM quality_cars
            WHERE amo_id = :amo_id
              AND due_date IS NOT NULL
        ...
        logger.error("QMS calendar source failed trace_id=%s label=car_due error=%s", trace_id, exc)
        source_errors.append({"label": "car_due", "message": str(exc), "type": exc.__class__.__name__})
        _recover_qms_read_session(db, amo_id=ctx.amo_id, user_id=ctx.user_id, timeout_ms=5000)
        return []
    return [
        _event_with_state("cars", "car", str(row["id"]), str(row.get("title") or row.get("car_number") or "CAR due"), row["due_date"], "car_due", f"/maintenance/{ctx.amo_code}/quality/cars/{row['id']}/overview", today=today)
        for row in rows
    ]


def _calendar_source_training(
    db: Session,
    *,
    ctx: TenantContext,
    start_date: date,
    ...
    try:
        _pg_set_read_timeout(db, 5000)
        training_record_columns = _table_columns(db, "training_records")
        lifecycle_filters: list[str] = []
        if "record_status" in training_record_columns:
            lifecycle_filters.append("COALESCE(UPPER(NULLIF(r.record_status, '')), 'ACTIVE') NOT IN ('RENEWED', 'SUPERSEDED')")
        if "source_status" in training_record_columns:
            lifecycle_filters.append("COALESCE(UPPER(NULLIF(r.source_status, '')), 'ACTIVE') NOT IN ('RENEWED', 'SUPERSEDED')")
        lifecycle_sql = "" if not lifecycle_filters else " AND " + " AND ".join(lifecycle_filters)
        rows = db.execute(text(f"""
            WITH latest AS (
                SELECT DISTINCT ON (r.user_id, r.course_id)
                    r.id, r.user_id, r.course_id, r.valid_until, r.completion_date, r.created_at,
            ...
            SELECT * FROM latest
            WHERE valid_until >= :start_date AND valid_until <= :end_date
            ORDER BY valid_until ASC, user_name ASC
            LIMIT :limit
        """), {"amo_id": ctx.amo_id, "start_date": start_date, "end_date": end_date, "limit": source_limit}).mappings().all()
    except Exception as exc:
        logger.error("QMS calendar source failed trace_id=%s label=training_expiry error=%s", trace_id, exc)
        source_errors.append({"label": "training_expiry", "message": str(exc), "type": exc.__class__.__name__})
        _recover_qms_read_session(db, amo_id=ctx.amo_id, user_id=ctx.user_id, timeout_ms=5000)
        return []
    events: list[dict[str, Any]] = []
    for row in rows:
        course = str(row.get("course_name") or row.get("course_code") or row.get("course_id") or "Training")
        course_code = str(row.get("course_code") or row.get("course_id") or "")
        user_name = str(row.get("user_name") or row.get("user_id") or "Personnel")
        title = f"{user_name}: {course} expires"
        item = _event_with_state(
            "training-competence",
            "training_record",
            str(row["id"]),
            title,
            row["valid_until"],
            "training_expiry",
            f"/maintenance/{ctx.amo_code}/training/competence/people/{row['user_id']}/course-history",
            today=today,
        )
        item.update({
    ...
    try:
        ...
        session_rows = db.execute(text("""
            ...
            LEFT JOIN training_courses c ON c.id = e.course_id AND c.amo_id = e.amo_id
            LEFT JOIN training_event_participants p ON p.event_id = e.id AND p.amo_id = e.amo_id
            WHERE e.amo_id = :amo_id
              AND e.starts_on IS NOT NULL
              AND e.starts_on >= :start_date
              AND e.starts_on <= :end_date
              AND e.status <> 'CANCELLED'
            GROUP BY e.id, e.title, e.starts_on, e.ends_on, e.status, c.course_id, c.course_name
            ORDER BY e.starts_on ASC, e.id ASC
            LIMIT :limit
        """), {"amo_id": ctx.amo_id, "start_date": start_date, "end_date": end_date, "limit": source_limit}).mappings().all()
        for row in session_rows:
            participant_count = int(row.get("participant_count") or 0)
            course_code = str(row.get("course_code") or row.get("course_name") or "TRAINING")
            title = f"{course_code} ({participant_count})" if participant_count else str(row.get("title") or course_code)
            item = _event_with_state(
                "training-competence",
                "training_event",
                str(row["id"]),
                ...
                row["starts_on"],
                "training_session",
                f"/maintenance/{ctx.amo_code}/training/competence/schedule?event_id={row['id']}",
                today=today,
                actionable=True,
            )
            item.update({
                "event_id": str(row["id"]),
                "course_code": course_code,
                "course_name": str(row.get("course_name") or course_code),
        ...
        logger.error("QMS calendar source failed trace_id=%s label=training_sessions error=%s", trace_id, exc)
        source_errors.append({"label": "training_sessions", "message": str(exc), "type": exc.__class__.__name__})
        _recover_qms_read_session(db, amo_id=ctx.amo_id, user_id=ctx.user_id, timeout_ms=5000)

    return events


@core_router.get("/calendar")
def qms_calendar(
    ...
    source_limit = min(max(bounded_limit + bounded_offset + 10, 50), 500)
    requested_source = (source or "all").strip().lower()
    source_errors: list[dict[str, Any]] = []
    events: list[dict[str, Any]] = []

    if requested_source in {"all", "list", "month", "week", "audits"}:
        events.extend(_calendar_source_audits(db, ctx=ctx, start_date=start_date, end_date=end_date, source_limit=source_limit, today=today, source_errors=source_errors, trace_id=trace_id))
    if requested_source in {"all", "list", "month", "week", "cars"}:
        events.extend(_calendar_source_cars(db, ctx=ctx, start_date=start_date, end_date=end_date, source_limit=source_limit, today=today, source_errors=source_errors, trace_id=trace_id))
    training_available = _training_calendar_available(db, amo_id=ctx.amo_id)
    if requested_source in {"all", "list", "month", "week", "training"} and training_available:
        events.extend(_calendar_source_training(db, ctx=ctx, start_date=start_date, end_date=end_date, source_limit=source_limit, today=today, source_errors=source_errors, trace_id=trace_id))
    elif requested_source == "training" and not training_available:
        source_errors.append({"label": "training", "message": "Training module is not enabled for this tenant.", "type": "ModuleNotEnabled"})

    events.sort(key=lambda item: (item.get("date") or "", item.get("title") or ""))
    visible = events[bounded_offset:bounded_offset + bounded_limit]
    elapsed_ms = round((time.perf_counter() - started) * 1000, 2)
    if source_errors and requested_source not in {"all", "list", "month", "week"} and not visible:
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail={"message": "QMS calendar source failed.", "trace_id": trace_id, "source_errors": source_errors})
    return {
        "module": "calendar",
...
def qms_calendar_view(
    ...
    source = view if view in {"audits", "cars", "training"} else None
...
# ---------------------------------------------------------------------------
# Fast module-integration endpoints
# ---------------------------------------------------------------------------

def _has_column_safe(db: Session, table: str, column: str) -> bool:
    try:
        return column in _table_columns(db, table)
    except Exception:
        return False


def _table_exists_safe(db: Session, table: str) -> bool:
    try:
...
def _tenant_has_module(db: Session, *, amo_id: str, module_key: str) -> bool:
    try:
        subscription = (
        ...
        if subscription is not None:
            status_value = getattr(subscription.status, "value", str(subscription.status))
            return status_value in {"ENABLED", "TRIAL"}
    except Exception:
        pass
    try:
        entitlement = account_services.resolve_entitlements(db, amo_id=amo_id).get(module_key)
        if entitlement is None:
            return False
        return bool(entitlement.is_unlimited or (entitlement.limit is not None and entitlement.limit > 0))
    except Exception:
        return False


def _training_calendar_available(db: Session, *, amo_id: str) -> bool:
    return (
        _tenant_has_module(db, amo_id=amo_id, module_key="training")
        and _table_exists_safe(db, "training_records")
        and _table_exists_safe(db, "training_events")
    )


def _soft_delete_filter(db: Session, table: str, alias: str = "") -> str:
    prefix = f"{alias}." if alias else ""
    return f" AND {prefix}deleted_at IS NULL" if _has_column_safe(db, table, "deleted_at") else ""


def _lite_scalar(db: Session, *, sql: str, params: dict[str, Any], source_errors: list[dict[str, Any]], label: str, ctx: TenantContext, timeout_ms: int = 1800) -> int:
    try:
        _pg_set_read_timeout(db, timeout_ms)
        return int(db.execute(text(sql), params).scalar() or 0)
    except Exception as exc:
        logger.error("QMS lite counter failed label=%s amo=%s error=%s", label, ctx.amo_code, exc)
        source_errors.append({"label": label, "message": str(exc), "type": exc.__class__.__name__})
        _recover_qms_read_session(db, amo_id=ctx.amo_id, user_id=ctx.user_id, timeout_ms=timeout_ms)
        return 0


@core_router.get("/dashboard-lite")
def qms_dashboard_lite(
    ctx: TenantContext = Depends(require_quality_permission("qms.dashboard.view")),
    db: Session = Depends(get_read_db),
) -> dict[str, Any]:
    """Fast dashboard read path used by the portal shell.

    This endpoint is intentionally narrow and resilient. It avoids heavyweight
    historical joins so login, QMS landing, calendar, and navigation do not go
    blank if one source table has a slow query or pending maintenance.
    """
    trace_id = uuid.uuid4().hex[:12]
    started = time.perf_counter()
    set_postgres_tenant_context(db, amo_id=ctx.amo_id, user_id=ctx.user_id)
    ...
    today = date.today()
    due_soon = today + timedelta(days=30)
    due_45 = today + timedelta(days=45)
    source_errors: list[dict[str, Any]] = []
    ...
    finding_deleted = _soft_delete_filter(db, "qms_audit_findings")

    params = {"amo_id": ctx.amo_id, "today": today, "due_soon": due_soon, "due_45": due_45}
    active_schedules = _lite_scalar(db, sql=f"""
        SELECT COUNT(*) FROM qms_audit_schedules
        WHERE amo_id = :amo_id AND is_active IS TRUE {schedule_deleted}
    """, params=params, source_errors=source_errors, label="active_audit_schedules", ctx=ctx)
    live_open_audits = _lite_scalar(db, sql=f"""
        SELECT COUNT(*) FROM qms_audits
        WHERE amo_id = :amo_id
          AND COALESCE(status, 'PLANNED') NOT IN ('CLOSED', 'CANCELLED') {audit_deleted}
    """, params=params, source_errors=source_errors, label="live_open_audits", ctx=ctx)
    due_schedule_soon = _lite_scalar(db, sql=f"""
        SELECT COUNT(*) FROM qms_audit_schedules
        WHERE amo_id = :amo_id AND is_active IS TRUE
          AND next_due_date IS NOT NULL AND next_due_date >= :today AND next_due_date <= :due_soon {schedule_deleted}
    """, params=params, source_errors=source_errors, label="audit_schedules_due_soon", ctx=ctx)
    due_live_soon = _lite_scalar(db, sql=f"""
        SELECT COUNT(*) FROM qms_audits
        WHERE amo_id = :amo_id
          AND planned_start IS NOT NULL AND planned_start >= :today AND planned_start <= :due_soon
          AND COALESCE(status, 'PLANNED') NOT IN ('CLOSED', 'CANCELLED') {audit_deleted}
    """, params=params, source_errors=source_errors, label="live_audits_due_soon", ctx=ctx)
    due_45_total = _lite_scalar(db, sql=f"""
        SELECT COUNT(*) FROM (
          SELECT id FROM qms_audit_schedules
          WHERE amo_id = :amo_id AND is_active IS TRUE
            AND next_due_date IS NOT NULL AND next_due_date >= :today AND next_due_date <= :due_45 {schedule_deleted}
          UNION ALL
          SELECT id FROM qms_audits
          WHERE amo_id = :amo_id
            AND planned_start IS NOT NULL AND planned_start >= :today AND planned_start <= :due_45
            AND COALESCE(status, 'PLANNED') NOT IN ('CLOSED', 'CANCELLED') {audit_deleted}
        ) x
    """, params=params, source_errors=source_errors, label="audits_due_45", ctx=ctx)
    active_fieldwork = _lite_scalar(db, sql=f"""
        SELECT COUNT(*) FROM qms_audits
        WHERE amo_id = :amo_id
          AND COALESCE(status, '') IN ('IN_PROGRESS', 'FIELDWORK', 'CAP_OPEN') {audit_deleted}
    """, params=params, source_errors=source_errors, label="active_audit_fieldwork", ctx=ctx)
    open_cars = _lite_scalar(db, sql=f"""
        SELECT COUNT(*) FROM quality_cars
        WHERE amo_id = :amo_id AND COALESCE(status, 'DRAFT') NOT IN ('CLOSED', 'CANCELLED') {car_deleted}
    """, params=params, source_errors=source_errors, label="open_cars", ctx=ctx)
    overdue_cars = _lite_scalar(db, sql=f"""
        SELECT COUNT(*) FROM quality_cars
        WHERE amo_id = :amo_id AND due_date IS NOT NULL AND due_date < :today
          AND COALESCE(status, 'DRAFT') NOT IN ('CLOSED', 'CANCELLED') {car_deleted}
    """, params=params, source_errors=source_errors, label="overdue_cars", ctx=ctx)
    cars_due_soon = _lite_scalar(db, sql=f"""
        SELECT COUNT(*) FROM quality_cars
        WHERE amo_id = :amo_id AND due_date IS NOT NULL AND due_date >= :today AND due_date <= :due_soon
          AND COALESCE(status, 'DRAFT') NOT IN ('CLOSED', 'CANCELLED') {car_deleted}
    """, params=params, source_errors=source_errors, label="cars_due_soon", ctx=ctx)
    open_findings = _lite_scalar(db, sql=f"""
        SELECT COUNT(*) FROM qms_audit_findings
        WHERE amo_id = :amo_id AND closed_at IS NULL {finding_deleted}
    """, params=params, source_errors=source_errors, label="open_findings", ctx=ctx)
    draft_docs = _lite_scalar(db, sql="""
        SELECT COUNT(*) FROM qms_documents
        WHERE amo_id = :amo_id AND status = 'DRAFT'
    """, params=params, source_errors=source_errors, label="draft_documents", ctx=ctx)
    active_docs = _lite_scalar(db, sql="""
        SELECT COUNT(*) FROM qms_documents
        WHERE amo_id = :amo_id AND status = 'ACTIVE'
    ...
    counters = {
        "open_audits": active_schedules + live_open_audits,
        "active_audit_schedules": active_schedules,
        "live_open_audits": live_open_audits,
        "audits_due_soon": due_schedule_soon + due_live_soon,
        "audit_programme_due_45": due_45_total,
        ...
        "training_expired_records": training_expired,
    }
    elapsed_ms = round((time.perf_counter() - started) * 1000, 2)
    return {
        "tenant": {"amo_code": ctx.amo_code, "amo_id": ctx.amo_id},
        "source": "tenant_scoped_backend_fast_path",
        "as_of": datetime.now(timezone.utc).isoformat(),
        "counters": counters,
        "counter_precision": {key: "exact" for key in counters},
        "source_errors": source_errors,
        "warning": "Some QMS counters failed. See source_errors." if source_errors else None,
        "links": {
            "open_cars": f"/maintenance/{ctx.amo_code}/quality/cars/overdue",
            "audits_due_soon": f"/maintenance/{ctx.amo_code}/quality/audits/plan?view=list",
            "documents": f"/maintenance/{ctx.amo_code}/quality/documents/library",
            "training": f"/maintenance/{ctx.amo_code}/training/competence/overdue",
        },
        "trace_id": trace_id,
...
def _calendar_event_row(
    *,
    ...
    item = _event_with_state(module, entity_type, str(entity_id), str(title or "Untitled"), event_date, event_type, link, today=today)
    if extra:
        item.update(extra)
    return item


def _append_calendar_rows(
    db: Session,
    *,
    ctx: TenantContext,
    label: str,
    ...
    try:
        ...
        logger.error("QMS integration calendar source failed label=%s amo=%s error=%s", label, ctx.amo_code, exc)
        source_errors.append({"label": label, "message": str(exc), "type": exc.__class__.__name__})
        _recover_qms_read_session(db, amo_id=ctx.amo_id, user_id=ctx.user_id, timeout_ms=timeout_ms)
        return []


@core_router.get("/integrations/calendar")
def qms_integration_calendar(
    start: date | None = Query(None),
    end: date | None = Query(None),
    ...
    if requested_source in {"all", "audits", "month", "week", "list", "agenda", "year"}:
        schedule_rows = _append_calendar_rows(db, ctx=ctx, label="audit_schedules", params=params, source_errors=source_errors, sql=f"""
            SELECT id, title, kind, frequency, auditee, lead_auditor_user_id, next_due_date AS event_date
            FROM qms_audit_schedules
            WHERE amo_id = :amo_id AND is_active IS TRUE
              AND next_due_date IS NOT NULL AND next_due_date >= :start_date AND next_due_date <= :end_date {schedule_deleted}
            ORDER BY next_due_date ASC, id ASC
            LIMIT :limit
        """)
        for row in schedule_rows:
            ...
            events.append(_calendar_event_row(
                module="audits", entity_type="audit_schedule", entity_id=row.get("id"), title=title,
                event_date=row.get("event_date"), event_type="audit_due",
                link=f"/maintenance/{ctx.amo_code}/quality/audits/plan?view=list&schedule_id={row.get('id')}", today=today,
                extra={
        ...
        audit_rows = _append_calendar_rows(db, ctx=ctx, label="live_audits", params=params, source_errors=source_errors, sql=f"""
            SELECT id, audit_ref, title, kind, status, auditee, lead_auditor_user_id, planned_start AS event_date, planned_end
            FROM qms_audits
            WHERE amo_id = :amo_id
              AND planned_start IS NOT NULL AND planned_start >= :start_date AND planned_start <= :end_date
              AND COALESCE(status, 'PLANNED') NOT IN ('CLOSED', 'CANCELLED') {audit_deleted}
    ...
    if requested_source in {"all", "cars", "month", "week", "list", "agenda", "year"}:
        car_rows = _append_calendar_rows(db, ctx=ctx, label="cars", params=params, source_errors=source_errors, sql=f"""
            SELECT id, car_number, title, priority, status, due_date AS event_date
            FROM quality_cars
            WHERE amo_id = :amo_id
              AND due_date IS NOT NULL AND due_date >= :start_date AND due_date <= :end_date
              AND COALESCE(status, 'DRAFT') NOT IN ('CLOSED', 'CANCELLED') {car_deleted}
            ORDER BY due_date ASC, id ASC
            LIMIT :limit
        """)
        for row in car_rows:
            ...
            events.append(_calendar_event_row(
                module="cars", entity_type="car", entity_id=row.get("id"), title=title,
                event_date=row.get("event_date"), event_type="car_due",
                link=f"/maintenance/{ctx.amo_code}/quality/cars/{row.get('id')}/overview", today=today,
                extra={"calendar_group": "car", "status": row.get("status"), "priority": row.get("priority"), "car_number": row.get("car_number")},
            ))
    ...
    if requested_source in {"all", "training", "month", "week", "list", "agenda", "year"} and training_available:
        ...
        for row in training_rows:
            ...
            events.append(_calendar_event_row(
                module="training-competence", entity_type="training_record", entity_id=row.get("id"), title=title,
                event_date=row.get("event_date"), event_type="training_expiry",
                link=f"/maintenance/{ctx.amo_code}/training/competence/people/{row.get('user_id')}/course-history", today=today,
                extra={"calendar_group": "training", "course_code": course, "user_id": row.get("user_id"), "user_name": row.get("user_name")},
            ))
        session_rows = _append_calendar_rows(db, ctx=ctx, label="training_sessions", params=params, source_errors=source_errors, timeout_ms=1400, sql="""
            SELECT e.id, e.title, e.starts_on AS event_date, e.ends_on, e.status,
                   c.course_id AS course_code, c.course_name
            FROM training_events e
            LEFT JOIN training_courses c ON c.id = e.course_id AND c.amo_id = e.amo_id
            WHERE e.amo_id = :amo_id
              AND e.starts_on IS NOT NULL AND e.starts_on >= :start_date AND e.starts_on <= :end_date
        ...
        for row in session_rows:
            ...
            events.append(_calendar_event_row(
                module="training-competence", entity_type="training_event", entity_id=row.get("id"), title=course,
                event_date=row.get("event_date"), event_type="training_session",
                link=f"/maintenance/{ctx.amo_code}/training/competence/schedule?event_id={row.get('id')}", today=today,
                extra={"calendar_group": "training", "course_code": course, "status": row.get("status"), "ends_on": _as_date(row.get("ends_on"))},
            ))
...
@core_router.get("/documents")
def list_documents(
    limit: int = Query(100, ge=1, le=500),
    ctx: TenantContext = Depends(require_quality_permission("qms.document.view")),
    db: Session = Depends(get_read_db),
) -> dict[str, Any]:
    set_postgres_tenant_context(db, amo_id=ctx.amo_id, user_id=ctx.user_id)
    rows = (
...
@core_router.get("/training-competence/dashboard")
...
@core_router.get("/settings")
def get_qms_settings(
    ctx: TenantContext = Depends(require_quality_permission("qms.settings.view")),
    db: Session = Depends(get_read_db),
) -> dict[str, Any]:
    set_postgres_tenant_context(db, amo_id=ctx.amo_id, user_id=ctx.user_id)
    row = db.execute(
...
def _ensure_postgres(db: Session) -> None:
    if not _is_postgres(db):
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Canonical Quality modules require PostgreSQL.")
...
def qms_route_map(
    ...
    return {
        "base_path": f"/maintenance/{ctx.amo_code}/quality",
        "api_base_path": f"/api/maintenance/{ctx.amo_code}/quality",
        "tenant": {"amo_code": ctx.amo_code, "amo_id": ctx.amo_id},
        "modules": modules,
    }


@core_router.patch("/settings")
...
@core_router.get("/evidence-vault/files/{file_id}/download")
def download_evidence_file(
    file_id: str,
    request: Request,
    ctx: TenantContext = Depends(require_quality_permission("qms.evidence.download")),
    db: Session = Depends(get_write_db),
):
    set_postgres_tenant_context(db, amo_id=ctx.amo_id, user_id=ctx.user_id)
    row = db.execute(
...
@core_router.post("/{module}/{record_id}/{action}")
def qms_workflow_action(
    ...
    assert_quality_permission(db, ctx, str(action_config["permission"]))
    set_postgres_tenant_context(db, amo_id=ctx.amo_id, user_id=ctx.user_id)
...
def generic_qms_get(
    ...
    if not parts:
        assert_quality_permission(db, ctx, "qms.dashboard.view")
        return qms_dashboard(ctx=ctx, db=db)
    module = parts[0]
    config = _module_config(module)
    assert_quality_permission(db, ctx, str(config["permission"]))
    set_postgres_tenant_context(db, amo_id=ctx.amo_id, user_id=ctx.user_id)
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail={
                "message": f"QMS read failed for {module}/{view or 'default'} before a usable response was returned.",
                "trace_id": trace_id,
                "module": module,
...
@core_router.post("/{module_path:path}")
def generic_qms_create(
    ...
    module = parts[0]
    config = _module_config(module)
    assert_quality_permission(db, ctx, str(config.get("manage_permission") or config["permission"]))
    set_postgres_tenant_context(db, amo_id=ctx.amo_id, user_id=ctx.user_id)
    view = _resolve_view(module, parts[1:])
    table = _resolve_table(module, view)
...
@core_router.patch("/{module_path:path}")
...
@core_router.delete("/{module_path:path}")
def generic_qms_delete(
    ...
    module = parts[0]
    config = _module_config(module)
    assert_quality_permission(db, ctx, str(config.get("manage_permission") or "qms.settings.manage"))
    view = _resolve_view(module, parts[1:])
    table = _resolve_table(module, view)
    record_id = _record_id_from_parts(parts[1:], table, module)
    if not record_id:
