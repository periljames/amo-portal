...
def _promote_people_routes(api_router: APIRouter) -> None:
    registered = [route_item for route_item in api_router.routes if _is_people_route(route_item)]
    if not registered:
        raise RuntimeError("QMS People & Privileges routes were not registered")
    ...
    people_routes = list(reversed(selected_reversed))

    remaining = [route_item for route_item in api_router.routes if not _is_people_route(route_item)]
    catchall_index = next(
        (index for index, route_item in enumerate(remaining) if _is_generic_catchall(route_item)),
        len(remaining),
    )
    api_router.routes[:] = [*remaining[:catchall_index], *people_routes, *remaining[catchall_index:]]
