...
def _full_metrics(db: Session, ctx: TenantContext) -> tuple[dict[str, int], list[dict[str, str]]]:
    ...
    queries = {
        ...
        "expired_training": "SELECT COUNT(*) FROM (SELECT DISTINCT ON (user_id,course_id) user_id,course_id,valid_until FROM training_records WHERE amo_id=:amo_id AND valid_until IS NOT NULL ORDER BY user_id,course_id,completion_date DESC NULLS LAST,created_at DESC NULLS LAST,id DESC) latest WHERE valid_until < :today",
