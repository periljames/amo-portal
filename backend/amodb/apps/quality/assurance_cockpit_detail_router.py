...
def unscheduled_programme_requirements(
    ...
    params: dict[str, Any] = {"amo_id": ctx.amo_id, "period": period, "limit": limit}
    if view == "mine":
        where.append("(p.owner_user_id = :actor_user_id OR i.lead_auditor_user_id = :actor_user_id OR i.observer_auditor_user_id = :actor_user_id OR i.auditee_user_id = :actor_user_id)")
    ...
    rows = db.execute(
        text(
            ...
            " ORDER BY i.mandatory_surveillance DESC, i.target_start NULLS LAST, i.title LIMIT :limit"
        ),
        params,
    ).mappings().all()
    items = []
