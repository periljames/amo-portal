...
class AircraftFamily(Base):
    ...
    __table_args__ = (UniqueConstraint("code", name="uq_aircraft_family_code"), Index("ix_aircraft_family_manufacturer", "manufacturer", "status"))
...
class AircraftType(Base):
    ...
    __table_args__ = (UniqueConstraint("family_id", "type_code", name="uq_aircraft_type_family_code"), Index("ix_aircraft_type_tc", "type_certificate_number", "authority"))
    id = Column(String(36), primary_key=True, default=generate_uuid7)
    ...
    name = Column(String(160), nullable=False)
    type_certificate_number = Column(String(128))
...
class AircraftVariant(Base):
    ...
    __table_args__ = (UniqueConstraint("aircraft_type_id", "variant_code", name="uq_aircraft_variant_type_code"), Index("ix_aircraft_variant_model_code", "model_code"))
    id = Column(String(36), primary_key=True, default=generate_uuid7)
    ...
    marketing_name = Column(String(160))
    description = Column(Text)
...
class AircraftTypeTemplate(Base):
    ...
    __table_args__ = (UniqueConstraint("code", name="uq_aircraft_type_template_code"), Index("ix_aircraft_type_template_scope", "visibility", "owner_amo_id", "status"))
    id = Column(String(36), primary_key=True, default=generate_uuid7)
    ...
    owner_amo_id = Column(String(36), ForeignKey("amos.id", ondelete="CASCADE"), index=True)
    description = Column(Text)
    status = Column(String(20), nullable=False, default="ACTIVE", index=True)
...
class AircraftTypeTemplateRevision(Base):
    ...
    __table_args__ = (UniqueConstraint("template_id", "revision_code", name="uq_aircraft_type_template_revision"), Index("ix_aircraft_type_revision_status", "template_id", "status", "effective_date"))
    id = Column(String(36), primary_key=True, default=generate_uuid7)
    ...
    approved_by_user_id = Column(String(36), ForeignKey("users.id", ondelete="SET NULL"))
    approved_at = Column(DateTime(timezone=True))
    created_by_user_id = Column(String(36), ForeignKey("users.id", ondelete="SET NULL"))
    created_at = Column(DateTime(timezone=True), nullable=False, default=utcnow)
    updated_at = Column(DateTime(timezone=True), nullable=False, default=utcnow, onupdate=utcnow)
    template = relationship("AircraftTypeTemplate", back_populates="revisions", lazy="joined")
    source_documents = relationship("TemplateSourceDocument", back_populates="template_revision", cascade="all, delete-orphan", lazy="selectin")
    configuration_nodes = relationship("TemplateConfigurationNode", back_populates="template_revision", cascade="all, delete-orphan", lazy="selectin")
    requirements = relationship("TemplateRequirement", back_populates="template_revision", cascade="all, delete-orphan", lazy="selectin")


class TemplateSourceDocument(Base):
    __tablename__ = "aircraft_template_source_documents"
    __table_args__ = (UniqueConstraint("template_revision_id", "document_type", "reference", "document_revision", name="uq_aircraft_template_source_document"), Index("ix_template_source_revision", "template_revision_id", "document_type"))
    id = Column(String(36), primary_key=True, default=generate_uuid7)
    template_revision_id = Column(String(36), ForeignKey("aircraft_type_template_revisions.id", ondelete="CASCADE"), nullable=False, index=True)
    document_type = Column(String(32), nullable=False)
    reference = Column(String(160), nullable=False)
    document_revision = Column(String(64))
    issue_date = Column(Date)
    authority = Column(String(32))
    source_uri = Column(String(512))
    content_hash = Column(String(64))
    notes = Column(Text)
    template_revision = relationship("AircraftTypeTemplateRevision", back_populates="source_documents", lazy="joined")


class TemplateConfigurationNode(Base):
    __tablename__ = "aircraft_template_configuration_nodes"
    __table_args__ = (UniqueConstraint("template_revision_id", "node_key", name="uq_aircraft_template_config_node"), Index("ix_template_config_revision_parent", "template_revision_id", "parent_node_key", "sequence_no"))
    id = Column(String(36), primary_key=True, default=generate_uuid7)
    template_revision_id = Column(String(36), ForeignKey("aircraft_type_template_revisions.id", ondelete="CASCADE"), nullable=False, index=True)
    node_key = Column(String(128), nullable=False)
    parent_node_key = Column(String(128))
    node_type = Column(String(32), nullable=False)
    position_code = Column(String(64))
    title = Column(String(255), nullable=False)
    ata_chapter = Column(String(20))
    ...
    template_revision = relationship("AircraftTypeTemplateRevision", back_populates="configuration_nodes", lazy="joined")


class TemplateRequirement(Base):
    __tablename__ = "aircraft_template_requirements"
    __table_args__ = (UniqueConstraint("template_revision_id", "requirement_key", name="uq_aircraft_template_requirement"), Index("ix_template_requirement_revision_ata", "template_revision_id", "ata_chapter", "category"))
    id = Column(String(36), primary_key=True, default=generate_uuid7)
    template_revision_id = Column(String(36), ForeignKey("aircraft_type_template_revisions.id", ondelete="CASCADE"), nullable=False, index=True)
    requirement_key = Column(String(128), nullable=False)
    category = Column(String(32), nullable=False, index=True)
    ata_chapter = Column(String(20), index=True)
    ...
    effectivity_json = Column(JSON, nullable=False, default=dict)
    source_reference = Column(String(255))
    source_document_id = Column(String(36), ForeignKey("aircraft_template_source_documents.id", ondelete="SET NULL"))
    mandatory = Column(Boolean, nullable=False, default=True)
    sequence_no = Column(Integer, nullable=False, default=1)
    template_revision = relationship("AircraftTypeTemplateRevision", back_populates="requirements", lazy="joined")


class ImportMappingProfile(Base):
    __tablename__ = "aircraft_import_mapping_profiles"
    __table_args__ = (UniqueConstraint("scope", "amo_id", "name", "version", name="uq_aircraft_import_mapping_profile"), Index("ix_import_mapping_fingerprint", "dataset", "fingerprint", "status"), Index("ix_import_mapping_source", "source_system", "source_version", "dataset"))
    id = Column(String(36), primary_key=True, default=generate_uuid7)
    amo_id = Column(String(36), ForeignKey("amos.id", ondelete="CASCADE"), index=True)
...
class TenantMaintenanceProgram(Base):
    ...
    __table_args__ = (UniqueConstraint("amo_id", "code", name="uq_tenant_maintenance_program_code"), Index("ix_tenant_program_variant", "amo_id", "variant_id", "status"))
    id = Column(String(36), primary_key=True, default=generate_uuid7)
    ...
    title = Column(String(255), nullable=False)
    authority = Column(String(32))
    approval_reference = Column(String(160))
    status = Column(String(20), nullable=False, default="ACTIVE", index=True)
    created_by_user_id = Column(String(36), ForeignKey("users.id", ondelete="SET NULL"))
    created_at = Column(DateTime(timezone=True), nullable=False, default=utcnow)
    updated_at = Column(DateTime(timezone=True), nullable=False, default=utcnow, onupdate=utcnow)
...
class TenantMaintenanceProgramRevision(Base):
    ...
    __table_args__ = (UniqueConstraint("program_id", "revision_code", name="uq_tenant_program_revision"), Index("ix_tenant_program_revision_status", "program_id", "status", "effective_date"))
    id = Column(String(36), primary_key=True, default=generate_uuid7)
    ...
    base_template_revision = relationship("AircraftTypeTemplateRevision", lazy="joined")
    overrides = relationship("TenantProgramOverride", back_populates="program_revision", cascade="all, delete-orphan", lazy="selectin")


class TenantProgramOverride(Base):
    __tablename__ = "tenant_program_overrides"
    __table_args__ = (UniqueConstraint("program_revision_id", "requirement_key", name="uq_tenant_program_override"), Index("ix_tenant_program_override_action", "program_revision_id", "action"))
    id = Column(String(36), primary_key=True, default=generate_uuid7)
    ...
    authority_reference = Column(String(160))
    program_revision = relationship("TenantMaintenanceProgramRevision", back_populates="overrides", lazy="joined")


class AircraftInduction(Base):
    __tablename__ = "aircraft_inductions"
    __table_args__ = (UniqueConstraint("amo_id", "induction_ref", name="uq_aircraft_induction_ref"), Index("ix_aircraft_induction_status", "amo_id", "status", "created_at"), Index("ix_aircraft_induction_aircraft", "amo_id", "serial_number", "registration"))
    id = Column(String(36), primary_key=True, default=generate_uuid7)
    amo_id = Column(String(36), ForeignKey("amos.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    created_at = Column(DateTime(timezone=True), nullable=False, default=utcnow)
    updated_at = Column(DateTime(timezone=True), nullable=False, default=utcnow, onupdate=utcnow)
    variant = relationship("AircraftVariant", lazy="joined")
    template_revision = relationship("AircraftTypeTemplateRevision", lazy="joined")
    program_revision = relationship("TenantMaintenanceProgramRevision", lazy="joined")
...
class AircraftInductionDataset(Base):
    ...
    __table_args__ = (UniqueConstraint("induction_id", "dataset", "source_name", "source_sheet", name="uq_aircraft_induction_dataset"), Index("ix_aircraft_induction_dataset_status", "induction_id", "status"))
    id = Column(String(36), primary_key=True, default=generate_uuid7)
    induction_id = Column(String(36), ForeignKey("aircraft_inductions.id", ondelete="CASCADE"), nullable=False, index=True)
    dataset = Column(String(32), nullable=False, index=True)
    source_name = Column(String(255), nullable=False)
    source_sheet = Column(String(160))
    fingerprint = Column(String(64), nullable=False, index=True)
    mapping_profile_id = Column(String(36), ForeignKey("aircraft_import_mapping_profiles.id", ondelete="SET NULL"))
    ...
    mapping_profile = relationship("ImportMappingProfile", lazy="joined")
...
class AircraftInductionRow(Base):
    ...
    __table_args__ = (UniqueConstraint("dataset_id", "row_number", name="uq_aircraft_induction_row"), Index("ix_aircraft_induction_row_status", "dataset_id", "status", "row_number"))
    id = Column(String(36), primary_key=True, default=generate_uuid7)
    ...
    decided_by_user_id = Column(String(36), ForeignKey("users.id", ondelete="SET NULL"))
...
class AircraftApplicabilitySnapshot(Base):
    ...
    __table_args__ = (Index("ix_aircraft_applicability_aircraft", "amo_id", "aircraft_serial_number", "created_at"), Index("ix_aircraft_applicability_hash", "snapshot_hash"))
    id = Column(String(36), primary_key=True, default=generate_uuid7)
    amo_id = Column(String(36), ForeignKey("amos.id", ondelete="CASCADE"), nullable=False, index=True)
    induction_id = Column(String(36), ForeignKey("aircraft_inductions.id", ondelete="SET NULL"), index=True)
...
class AircraftTemplateBinding(Base):
    ...
    __table_args__ = (Index("ix_aircraft_template_binding_active", "amo_id", "aircraft_serial_number", "status"),)
    id = Column(String(36), primary_key=True, default=generate_uuid7)
    amo_id = Column(String(36), ForeignKey("amos.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    activated_by_user_id = Column(String(36), ForeignKey("users.id", ondelete="SET NULL"))
...
class InductionCounterBaseline(Base):
    ...
    __table_args__ = (UniqueConstraint("induction_id", "counter_code", name="uq_induction_counter_baseline"), CheckConstraint("value >= 0", name="ck_induction_counter_baseline_nonnegative"))
    id = Column(String(36), primary_key=True, default=generate_uuid7)
    induction_id = Column(String(36), ForeignKey("aircraft_inductions.id", ondelete="CASCADE"), nullable=False, index=True)
