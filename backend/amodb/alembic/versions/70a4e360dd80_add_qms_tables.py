...
def upgrade() -> None:
    ...
    op.execute("ALTER TABLE aircraft DROP CONSTRAINT IF EXISTS aircraft_registration_key")

    # Make uq_aircraft_registration idempotent:
    # - If constraint exists: do nothing
    # - Else if unique index exists with same name: attach constraint USING INDEX
    # - Else create constraint normally
    uq_name = op.f('uq_aircraft_registration')
    op.execute(f"""
DO $$
BEGIN
  IF EXISTS (
  ...
  ELSIF EXISTS (
    ...
    WHERE ic.relname = '{uq_name}'
      AND ix.indisunique
  ) THEN
    EXECUTE format('ALTER TABLE aircraft ADD CONSTRAINT %I UNIQUE USING INDEX %I', '{uq_name}', '{uq_name}');

  ELSIF EXISTS (
    SELECT 1
    FROM pg_class
    WHERE relkind = 'i' AND relname = '{uq_name}'
  ) THEN
    EXECUTE format('DROP INDEX %I', '{uq_name}');
    EXECUTE format('ALTER TABLE aircraft ADD CONSTRAINT %I UNIQUE (registration)', '{uq_name}');
...
def downgrade() -> None:
    """Downgrade schema."""
    # ### commands auto generated by Alembic - please adjust! ###
    # Make downgrade safe if constraint/index already missing/present
    op.execute("ALTER TABLE aircraft DROP CONSTRAINT IF EXISTS uq_aircraft_registration")
    op.execute("DROP INDEX IF EXISTS uq_aircraft_registration")
