...
from amodb.apps.accounts import models as account_models  # noqa: F401
from amodb.apps.foundations import models as foundation_models  # noqa: F401
from amodb.apps.training import models as training_models  # noqa: F401
from amodb.apps.work import models as work_models  # noqa: F401
from amodb.apps.rostering import models as roster_models
from amodb.apps.workforce import models as workforce_models  # noqa: F401
...
def _create_foreign_keys(bind) -> None:
    ...
    if dialect == "sqlite":
        # SQLite cannot add named FKs after table creation without a table copy.
        # Tests run against SQLite; PostgreSQL receives the production FKs below.
        return
...
def _execute_best_effort(bind, postgres_sql: str, sqlite_sql: str | None = None) -> None:
    try:
        ...
        # The migration remains rerunnable across supported legacy snapshots.
        # Missing optional source columns are handled by later introspection.
        return
...
def _backfill_existing_rows(bind) -> None:
    ...
    bind.execute(text("UPDATE roster_validation_findings SET overridable = 0 WHERE overridable IS NULL"))
...
def _migrate_legacy_availability(bind) -> None:
    ...
    if not inspector.has_table("user_availability") or not inspector.has_table("employee_availability_events"):
        return
    columns = [str(column["name"]) for column in inspector.get_columns("user_availability")]
    if not columns:
        return
    rows = bind.execute(text("SELECT * FROM user_availability")).mappings().all()
    target = Base.metadata.tables["employee_availability_events"]
    existing_sources = {
        (str(row[0]), str(row[1]))
        for row in bind.execute(text("SELECT source_type, source_id FROM employee_availability_events WHERE source_type = 'LEGACY_QUALITY_AVAILABILITY'")).fetchall()
    }
    ...
    for index, row in enumerate(rows, start=1):
        ...
        starts_at = _as_aware(_first(mapping, ("starts_at", "start_at", "valid_from", "start_date", "from_date")))
        ends_at = _as_aware(_first(mapping, ("ends_at", "end_at", "valid_to", "end_date", "to_date")), end_of_day=True)
