...
REQUIRED: dict[Path, tuple[str, ...]] = {
    ...
    BACKEND_ROOT / "amodb/jobs/saas_worker.py": (
        ...
        "cryptographically verified tenant",
        "declared tenant does not match the verified tenant",
        "account_models.BillingInvoice.amo_id == tenant_id",
        "Portal invoice does not belong to the verified Stripe tenant",
        "Non-repeatable side effects must run through amodb.jobs.saas_worker_safe",
    ),
    ...
    BACKEND_ROOT / "amodb/apps/platform/saas_webhooks.py": (
        ...
        "Stripe webhook tenant resolution is ambiguous",
    ...
    BACKEND_ROOT / "amodb/apps/platform/tenant_saas_router.py": (
        ...
        "def _queue_summary",
        "models.SaaSJob.tenant_id == tenant_id",
...
def main() -> int:
    ...
    managed_ai_order = package_init.index("install_managed_ai_provider_policy()") < package_init.index("from .saas_router import")
    passed = passed and policy_order and managed_ai_order
