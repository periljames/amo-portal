"""Shared admission controls for horizontally replicated public auth routes."""
...
class RedisAuthRateLimiter:
    ...
    def __init__(self) -> None:
        self.required = env_bool("AUTH_RATE_LIMIT_SHARED_REQUIRED", False)
        self.url = (os.getenv("AUTH_RATE_LIMIT_REDIS_URL") or os.getenv("REDIS_URL") or "").strip()
        self.window_seconds = bounded_int("AUTH_RATE_LIMIT_WINDOW_SEC", 60, 10, 3600)
        self.max_attempts = bounded_int("AUTH_RATE_LIMIT_MAX_ATTEMPTS", 10, 2, 1000)
...
class DistributedAuthRateLimitMiddleware:
    def __init__(self, app: ASGIApp, *, limiter: RedisAuthRateLimiter) -> None:
        ...
        self.max_body_bytes = bounded_int("AUTH_RATE_LIMIT_BODY_MAX_BYTES", 65536, 4096, 1048576)

    @staticmethod
    def subject(endpoint: str, body: bytes, scope: Scope) -> str:
        try:
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        if scope.get("type") != "http" or str(scope.get("method") or "").upper() != "POST":
            ...
            return
        endpoint = PROTECTED_AUTH_PATHS.get(str(scope.get("path") or ""))
        if not endpoint:
            await self.app(scope, receive, send)
            return
        ...
        if len(body) > self.max_body_bytes:
            await self.respond(send, 413, "Authentication request body is too large.")
            return

        subject = self.subject(endpoint, body, scope)
        try:
