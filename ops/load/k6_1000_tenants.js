...
const uniqueTenantIds = new Set(tenants.map((item) => String(item?.tenant_id || "")));
if (uniqueTenantIds.size < 1000 || uniqueTenantIds.has("")) {
  throw new Error(`The 1,000-tenant gate requires 1,000 distinct non-empty tenant_id values; received ${uniqueTenantIds.size}.`);
}
...
export default function () {
  ...
  const params = {
    ...
    tags: { tenant_id: String(fixture.tenant_id) },
    timeout: "10s",
  };
  ...
  check(responses[2], { "entitlements succeeds": (r) => r.status === 200 });
  sleep(1);
}
