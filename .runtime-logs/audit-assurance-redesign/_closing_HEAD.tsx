...
const AuditClosingWorkspace: React.FC<Props> = ({ amoCode, auditKey }) => {
  ...
  if (auditQuery.isLoading || compositionQuery.isLoading || revisionsQuery.isLoading || closureQuery.isLoading || policyQuery.isLoading || signaturesQuery.isLoading || acknowledgementsQuery.isLoading || assuranceArtifactsQuery.isLoading || (canManage && passkeysQuery.isLoading)) return <div className="qms-audit-closing qms-audit-closing--loading">Preparing governed closing meeting workspaceΓÇª</div>;
  ...
  return (
    <div className="qms-audit-closing" role="region" aria-label="Audit closing meeting workspace">
      <header className="qms-audit-closing__header">
        <div><span>CLOSING MEETING ┬╖ acknowledge ΓåÆ approve ΓåÆ passkey ΓåÆ issue</span><h1>{auditQuery.data.audit_ref} ┬╖ {auditQuery.data.title}</h1></div>
      ...
      <div className="qms-audit-closing__body"><main>
        <section className="qms-audit-closing__card">
          ...
          <div className="qms-audit-closing__actions"><button type="button" className="is-primary" disabled={!canGenerate || generateMutation.isPending} onClick={() => generateMutation.mutate()}>{generateMutation.isPending ? "GeneratingΓÇª" : "Generate closing report draft"}</button>{latestGenerated ? <button type="button" onClick={() => void download(latestGenerated)} disabled={downloadBusy === latestGenerated.id}><Download size={15} /> {downloadBusy === latestGenerated.id ? "DownloadingΓÇª" : "Preview / download"}</button> : null}{latestGenerated && !activeRevision ? <button type="button" disabled={!canManage || adoptMutation.isPending} onClick={() => adoptMutation.mutate(latestGenerated.id)}><FileSignature size={15} /> {adoptMutation.isPending ? "AdoptingΓÇª" : "Adopt governed draft"}</button> : null}</div>
        ...
        <section className="qms-audit-closing__card">
          ...
          {!passkeys.length ? <div className="qms-audit-closing__passkey-setup"><label><span>Passkey label</span><input value={passkeyNickname} onChange={(event) => setPasskeyNickname(event.target.value)} maxLength={80} /></label><button type="button" disabled={!canManage || ceremonyBusy !== null} onClick={() => void registerPasskey()}><KeyRound size={15} /> {ceremonyBusy === "register" ? "RegisteringΓÇª" : "Register passkey"}</button></div> : <p>{passkeys.length} active passkey{passkeys.length === 1 ? "" : "s"} registered for this Quality user.</p>}
          {activeRevision?.status === "APPROVED" && !currentSignature ? <div className="qms-audit-closing__passkey-sign"><label><span>Approval reason</span><textarea rows={3} value={signReason} onChange={(event) => setSignReason(event.target.value)} /></label><button type="button" className="is-primary" disabled={!canSign || !passkeys.length || ceremonyBusy !== null || signReason.trim().length < 8} onClick={() => void signWithPasskey()}><Fingerprint size={15} /> {ceremonyBusy === "sign" ? "Verifying passkeyΓÇª" : "Approve exact report with passkey"}</button></div> : null}
          {currentSignature ? <dl className="qms-audit-closing__artifact"><div><dt>Method</dt><dd>{currentSignature.method}</dd></div><div><dt>Signed</dt><dd>{currentSignature.signed_at ? new Date(currentSignature.signed_at).toLocaleString() : "ΓÇö"}</dd></div><div className="is-wide"><dt>Ceremony SHA-256</dt><dd><code>{currentSignature.ceremony_sha256 || currentSignature.signature_digest}</code></dd></div></dl> : null}
        </section>
        ...
        <section className="qms-audit-closing__card">
          ...
          <button type="button" disabled={!canExecutionClose || executionCloseMutation.isPending} onClick={() => executionCloseMutation.mutate()}>{executionCloseMutation.isPending ? "Closing executionΓÇª" : closure?.execution_status === "CLOSED" ? "Execution closed" : "Close audit execution"}</button>
        </section>
        ...
        <section className="qms-audit-closing__card">
          ...
          {canGenerateAssurance && currentSignature ? <button type="button" disabled={assuranceArtifactMutation.isPending} onClick={() => assuranceArtifactMutation.mutate(currentSignature.id)}>{assuranceArtifactMutation.isPending ? "GeneratingΓÇª" : `Generate ${policy?.artifact_policy.replaceAll("_", " ")}`}</button> : null}
          ...
          {issuedRevision && currentSignature ? <button type="button" disabled={ceremonyBusy !== null} onClick={() => void createVerification()}><ExternalLink size={15} /> {ceremonyBusy === "verify-link" ? "CreatingΓÇª" : "Create public verification link"}</button> : null}
