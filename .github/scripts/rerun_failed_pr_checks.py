...
workflow runs associated with the PR's current head SHA.
"""
...
def pull_request_runs(api: GitHubAPI, head_sha: str) -> list[dict[str, Any]]:
    ...
    return [row for row in rows if str(row.get("head_sha") or "") == head_sha]
...
def main() -> int:
    ...
    rows = pull_request_runs(api, initial_head)
    failed = [
        row
        for row in rows
