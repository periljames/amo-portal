...
const resolveDevProxy = (env: Record<string, string>) => {
  ...
  return {
    '^/(auth|accounts|admin|billing|aircraft|work-orders|crs|training|public|quality|qms|reliability|audit|audit-events|bootstrap|integrations|api|notifications|email-logs|tasks|health|time)(?:/|$|\\?)': {
      target,
      changeOrigin: true,
      secure: false,
    },
