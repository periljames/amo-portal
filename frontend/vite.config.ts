...
const resolveDevProxy = (env: Record<string, string>) => {
  ...
  return {
    '^/(auth|accounts|admin|billing|aircraft|work-orders|crs|training|public|quality|qms|platform|reliability|audit|audit-events|bootstrap|integrations|api|notifications|email-logs|tasks|health|healthz|time|manuals|records|foundations|rostering)(?:/|$|\\?)': {
      target,
      changeOrigin: true,
      secure: false,
    },
