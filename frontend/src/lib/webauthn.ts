...
export const decodeCreationOptions = (options: Record<string, unknown>): PublicKeyCredentialCreationOptions => {
  ...
  return {
    ...
    user: { ...(parsed.user as unknown as PublicKeyCredentialUserEntity), id: b64ToBytes(parsed.user.id) },
    excludeCredentials: (parsed.excludeCredentials || []).map((credential) => ({ ...credential, id: b64ToBytes(credential.id) })),
  };
};
...
export const decodeRequestOptions = (options: Record<string, unknown>): PublicKeyCredentialRequestOptions => {
  ...
  return {
    ...
    challenge: b64ToBytes(parsed.challenge),
    allowCredentials: (parsed.allowCredentials || []).map((credential) => ({ ...credential, id: b64ToBytes(credential.id) })),
  };
};

export const createCredential = async (options: PublicKeyCredentialCreationOptions): Promise<PublicKeyCredential | null> =>
  (await navigator.credentials.create({ publicKey: options })) as PublicKeyCredential | null;
