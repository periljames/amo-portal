...
const rolePermissions: Record<string, string[]> = {
  // Platform superusers are intentionally excluded from tenant Quality navigation.
  AMO_ADMIN: ["qms.*"],
  QUALITY_MANAGER: ["qms.*"],
  QUALITY_INSPECTOR: [
    "qms.dashboard.view",
...
const groups: Array<{ heading: string; items: QmsNavItem[] }> = [
  {
    heading: "Command",
    items: [
      { label: "Quality Cockpit", path: "", permission: "qms.dashboard.view", exact: true },
      { label: "My Quality Work", path: "inbox", permission: "qms.inbox.view" },
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Audits", path: "audits/dashboard", permission: "qms.audit.view" },
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Quality Settings", path: "settings", permission: "qms.settings.view" },
    ],
  },
];
...
export default function QmsShell({ children }: QmsShellProps): React.ReactElement {
  ...
  const base = `/maintenance/${amoCode}/quality`;
  ...
  return (
    <div className="qms-shell" data-canonical-route={base}>
      <aside className="qms-shell__sidebar" aria-label="Quality navigation">
        <div className="qms-shell__brand">
          <span className="qms-shell__eyebrow">Canonical Quality</span>
              ...
              {visibleItems.map((item) => {
                const to = item.path
                  ? item.path.startsWith("../")
                    ? `/maintenance/${amoCode}/${item.path.slice(3)}`
                    : `${base}/${item.path}`
                  : base;
                return (
                  <NavLink
      ...
      <main className="qms-shell__main">
        <nav className="qms-shell__breadcrumbs" aria-label="Breadcrumb">
          <NavLink to={base}>Quality</NavLink>
