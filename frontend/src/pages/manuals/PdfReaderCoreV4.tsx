...
import "./pdfReaderDenseToolbar.css";
      ...
      <div className="pdfv3-page-surface" aria-hidden={!ready}>
        <PdfPage
          ...
          onLoadSuccess={(loaded: any) => {
            ...
            onMetrics(page, originalHeight / originalWidth, originalWidth);
          }}
          onRenderSuccess={() => {
            setFailed("");
            setReady(true);
...
export default function PdfReaderCoreV4({
  ...
  useEffect(() => {
    ...
    onZoomChange?.(fitMode === "CUSTOM" ? zoom : percent);
  }, [currentActualWidth, fitMode, onZoomChange, pageWidth, zoom]);
  ...
  useEffect(() => {
    const synchronizeFullscreen = () => setFullscreen(document.fullscreenElement === hostRef.current);
  ...
  const copyCurrentView = async (): Promise<void> => {
    ...
    window.setTimeout(() => setViewLinkCopied(false), 1_500);
  };
  ...
  const toggleFullscreen = async (): Promise<void> => {
    ...
    await hostRef.current?.requestFullscreen?.().catch(() => undefined);
  };

  const toggleDocumentNavigation = (): void => {
    const readerPage = hostRef.current?.closest<HTMLElement>(".publication-reader-page");
    if (!readerPage) return;
    const collapsed = readerPage.classList.contains("publication-reader-page--nav-collapsed");
    const control = collapsed
      ? readerPage.querySelector<HTMLButtonElement>(".publication-nav-restore, .publication-navigation-rail")
      : readerPage.querySelector<HTMLButtonElement>(".publication-toc__hide");
    control?.click();
  };
  ...
  const adjustZoom = (step: number): void => {
    const percent = fitMode === "CUSTOM"
      ? zoom
      : Math.round((pageWidth / Math.max(1, currentActualWidth)) * 100);
    setFitMode("CUSTOM");
    setZoom(clampPdfZoom(percent + step));
  };

  const scaleValue = fitMode === "CUSTOM" ? String(zoom) : fitMode;
  const hasCustomStep = !PDF_ZOOM_STEPS.includes(zoom as (typeof PDF_ZOOM_STEPS)[number]);
  ...
  return (
    <section
      ref={hostRef}
      className={`pdfv3-reader pdfv4-reader${compact ? " is-compact" : ""}${safeForm ? " is-form-active" : ""}`}
    ...
    >
      <header className="pdfv3-toolbar" aria-label="PDF reader toolbar">
        <div className="pdfv3-pages">
          <button
            type="button"
            aria-label="Show or hide document navigation"
            title="Show or hide document navigation"
            onClick={toggleDocumentNavigation}
          >
            <PanelLeft size={16} />
          </button>
          <button
        ...
        <div className="pdfv3-zoom">
          <button type="button" aria-label="Zoom out" title="Zoom out" onClick={() => adjustZoom(-10)}>
            <Minus size={17} />
          </button>
          <button type="button" aria-label="Zoom in" title="Zoom in" onClick={() => adjustZoom(10)}>
            <Plus size={17} />
          </button>
          <select
            className="pdfv4-scale-select"
            aria-label="Zoom"
            title="Zoom"
            value={scaleValue}
            onChange={(event) => {
              const next = parsePdfScaleValue(event.target.value);
              setFitMode(next.mode);
              if (next.zoom !== undefined) setZoom(next.zoom);
            }}
          >
            <option value="AUTO">Automatic Zoom</option>
            <option value="ACTUAL">Actual Size</option>
            <option value="PAGE">Page Fit</option>
            <option value="WIDTH">Page Width</option>
            {hasCustomStep ? <option value={String(zoom)}>{zoom}%</option> : null}
        ...
        <div className="pdfv3-actions">
          <button
            ...
            title={viewLinkCopied ? "Current view link copied" : "Copy current view link"}
            onClick={() => void copyCurrentView()}
          >
            <Bookmark size={16} fill={viewLinkCopied ? "currentColor" : "none"} />
          </button>
          <button
            type="button"
          ...
          <details className="pdfv3-menu">
            <summary aria-label="Download" title="Download">
              <Download size={16} />
            </summary>
            <div>
              <button
          ...
          <details className="pdfv3-menu">
            <summary aria-label="Tools" title="Tools">
                ...
                >
                  <VirtualPdfPage
                    ...
                    onMetrics={(pageNumber, nextRatio, originalWidth) => {
                      ...
                      setPageOriginalWidths((values) => (
                        Math.abs((values[pageNumber] || 0) - originalWidth) < 0.01
                          ? values
                          : { ...values, [pageNumber]: originalWidth }
                      ));
                    }}
