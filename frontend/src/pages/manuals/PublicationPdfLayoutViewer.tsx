...
export default function PublicationPdfLayoutViewer({
    ...
    const load = () => {
      getPublicationReferences(identity.tenant, identity.manualId, identity.revisionId)
        .then((response) => {
          ...
          if (indexing(response.index)) timer = window.setTimeout(load, 1_400);
        })
        .catch(() => {
          if (active) timer = window.setTimeout(load, 3_500);
        });
    };
  ...
  const selectedReference = allReferences.find(
              ...
              {(referencesByPage.get(pageNumber) || []).map((reference) => {
                ...
                return (
                  <button
                    type="button"
                    key={reference.id}
                    className={`publication-reference-hotspot${
