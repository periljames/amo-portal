    ...
    <main className="qms-public-audit">
      ...
      <div className="qms-public-audit__content">
        ...
        {isExternalAuditor ? <ExternalAuditorFieldworkWorkspace /> : <>
          ...
          {closing?.available && closing.report ? <section className="qms-public-audit__card" aria-label="Closing meeting acknowledgement"><header><FileCheck2 size={19} /><div><strong>Closing report response</strong><small>Draft revision {closing.report.revision_no}</small></div></header>{closing.acknowledgement ? <div className="qms-public-audit__success"><CheckCircle2 size={15} /> {closing.acknowledgement.acknowledgement_status.replaceAll("_", " ")}{closing.acknowledgement.comments ? ` · ${closing.acknowledgement.comments}` : ""}</div> : <><label><span>Response</span><select value={closingStatus} onChange={(event) => setClosingStatus(event.target.value as AuditGuestClosingAcknowledgement["acknowledgement_status"])}><option value="ACKNOWLEDGED">Acknowledge closing draft</option><option value="COMMENTED">Comment on closing draft</option><option value="DECLINED_TO_ACKNOWLEDGE">Decline acknowledgement</option></select></label><label><span>Comments</span><textarea rows={4} value={closingComments} onChange={(event) => setClosingComments(event.target.value)} /></label><button type="button" disabled={closingBusy} onClick={() => void submitClosingAcknowledgement()}>{closingBusy ? "Recording…" : "Record closing response"}</button><p className="qms-public-audit__privacy-note">This records your response to the exact draft. It is not a waiver and does not mean you accept the findings.</p></>}</section> : null}

          {issuedReport?.available && issuedReport.report ? <section className="qms-public-audit__card" aria-label="Issued audit report"><header><FileText size={19} /><div><strong>Issued audit report</strong><small>Download and acknowledge the issued report.</small></div></header><dl><div><dt>File</dt><dd>{issuedReport.report.filename}</dd></div><div><dt>Revision</dt><dd>{issuedReport.report.revision_no}</dd></div><div><dt>Issued</dt><dd>{dateTime(issuedReport.report.issued_at)}</dd></div></dl><p>{issuedReport.acknowledgement_statement}</p><div><button type="button" disabled={reportBusy !== null} onClick={() => void downloadReport()}><Download size={15} /> {reportBusy === "download" ? "Downloading…" : "Download issued report"}</button>{!issuedReport.report.acknowledged_at ? <button type="button" disabled={reportBusy !== null} onClick={() => void acknowledgeReport()}>{reportBusy === "acknowledge" ? "Recording…" : "Acknowledge issued report"}</button> : <span>Receipt acknowledged {dateTime(issuedReport.report.acknowledged_at)}</span>}</div></section> : null}
        </>}
      </div>
    </main>
  );
