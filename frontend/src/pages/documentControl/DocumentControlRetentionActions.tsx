...
export default function DocumentControlRetentionActions({ tenant, detail, onChanged }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const [retention, evidence, admin] = await Promise.all([
        listDocumentRetention(tenant, detail.document.id),
        listDocumentEvidenceAssets(tenant, detail.document.id),
  ...
  }, [detail.document.id, retentionClass, tenant]);
  ...
  const sourceOptions = sourceType === "REVISION"
    ? revisions.map((row) => ({ id: row.id, label: `Revision ${row.revision_number}` }))
    : sourceType === "EVIDENCE_ASSET"
      ? assets.map((row) => ({ id: row.asset_id, label: `${row.filename} · ${row.category}` }))
      : [];
  ...
  return <div className="dc-form" data-testid="document-control-retention-actions">
    ...
    <form className="dc-grid" onSubmit={create}>
      <label><span>Governed source</span><select value={sourceType} onChange={(event) => { setSourceType(event.target.value as DocumentRetentionSourceType); setSourceId(""); }}><option value="DOCUMENT">Whole controlled document</option><option value="REVISION">Specific revision</option><option value="EVIDENCE_ASSET">Retained evidence asset</option></select></label>
      {sourceType !== "DOCUMENT" ? <label><span>Source record</span><select required value={sourceId} onChange={(event) => setSourceId(event.target.value)}><option value="">Select controlled source</option>{sourceOptions.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label> : null}
    ...
    {selected ? <section className="dc-form wide" aria-label="Selected retention governance">
      ...
      <div className="dc-form__actions">
        {!selected.legal_hold && selected.status !== "DISPOSED" ? <button type="button" className="dc-button" disabled={busy || !reason.trim()} onClick={() => void act(() => updateDocumentRetentionHold(tenant, selected.id, true, reason))}><LockKeyhole size={14} /> Place legal hold</button> : null}
        {selected.legal_hold ? <button type="button" className="dc-button" disabled={busy} onClick={() => void act(() => updateDocumentRetentionHold(tenant, selected.id, false, reason))}><Ban size={14} /> Release legal hold</button> : null}
        {["ACTIVE", "DUE", "REJECTED"].includes(selected.status) && !selected.legal_hold ? <button type="button" className="dc-button" disabled={busy || !reason.trim()} onClick={() => void act(() => requestDocumentDisposition(tenant, selected.id, reason))}><Trash2 size={14} /> Request disposition</button> : null}
        {selected.status === "DISPOSITION_REQUESTED" ? <><button type="button" className="dc-button dc-button--primary" disabled={busy || !reason.trim()} onClick={() => void act(() => decideDocumentDisposition(tenant, selected.id, "APPROVE", reason))}><CheckCircle2 size={14} /> Approve disposition</button><button type="button" className="dc-button" disabled={busy || !reason.trim()} onClick={() => void act(() => decideDocumentDisposition(tenant, selected.id, "REJECT", reason))}><Ban size={14} /> Reject</button></> : null}
      </div>
