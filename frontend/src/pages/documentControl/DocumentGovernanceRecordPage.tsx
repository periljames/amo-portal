        ...
        <div className="dgov-record" data-testid="document-governance-record">
          ...
          <section className="dgov-panel">
            ...
            {showAssignment && canControl ? <div className="dgov-assignment-form">
              ...
              {assignment.assignee_type === "ROLE" ? <label>Tenant access profile<select value={assignment.role} onChange={(event) => setAssignment({ ...assignment, role: event.target.value })}><option value="">Select active profile</option>{data.assignment_options.access_profiles.map((profile) => <option key={profile.id} value={profile.code}>{profile.name} · {profile.category.replaceAll("_", " ")}</option>)}</select><small>The stable profile code remains attached if the tenant later changes the displayed terminology.</small></label> : null}
