...
export default function DocumentControlPrimaryActions({
  detail,
  tenant,
  basePath,
  onChanged,
  ...
  return <>
    ...
    {canPublish ? (
      publishReady
        ...
        : <button type="button" className="dc-button" onClick={() => navigate(`${basePath}/library/${detail.document.id}?view=workflow`)}><Rocket size={14} /> Continue approval</button>
    ) : null}
...
function PropertiesDialog({ detail, tenant, onClose, onChanged }: { detail: DocumentDetailResponse; tenant: string; onClose: () => void; onChanged: () => void }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : "Document properties could not be saved.");
    } finally {
      setBusy(false);
    }
  };

  return <DialogShell title="Edit controlled document properties" description="Changes are tenant-scoped, audited, and do not alter immutable published revision content." busy={busy} onClose={onClose}>
...
function RevisionUploadDialog({ detail, tenant, onClose, onChanged }: { detail: DocumentDetailResponse; tenant: string; onClose: () => void; onChanged: () => void }) {
  ...
  const inspect = async (selected: File | null) => {
    ...
    try {
      ...
      setPreview(result);
      setIssue(result.metadata.issue_number || latest?.issue_number || "00");
      setRevision(result.metadata.revision_number || "");
      setEffectiveDate(result.metadata.effective_date || "");
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "The revision source could not be inspected.");
    } finally {
      setBusy(false);
    }
  };
  ...
  const submit = async (event: FormEvent) => {
    ...
    setError("");
    try {
      await uploadPublicationRevision(tenant, {
        code: detail.document.code,
        title: detail.document.title,
        rev_number: revision.trim(),
        issue_number: issue.trim(),
        effective_date: effectiveDate || undefined,
        manual_type: detail.document.manual_type,
        owner_role: detail.document.owner_role,
        change_log: changeNote.trim() || undefined,
        file,
      });
      onChanged();
      onClose();
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "The controlled revision could not be uploaded.");
    } finally {
      setBusy(false);
    }
  };

  return <DialogShell title="Upload a new controlled revision" description="The source remains an uncontrolled draft until it completes the approval workflow." busy={busy} onClose={onClose}>
    <form className="dc-form" onSubmit={submit}>
      ...
      <label><span>Proposed effective date</span><input type="date" value={effectiveDate} onChange={(event) => setEffectiveDate(event.target.value)} /></label>
      <label className="wide"><span>Change summary</span><textarea value={changeNote} onChange={(event) => setChangeNote(event.target.value)} required /></label>
      ...
      {error ? <div className="dc-form__error">{error}</div> : null}
      <div className="dc-form__actions"><button type="button" className="dc-button" onClick={onClose} disabled={busy}>Cancel</button><button type="submit" className="dc-button dc-button--primary" disabled={busy || !file || !revision.trim() || !changeNote.trim()}>{busy ? "Uploading…" : "Create draft revision"}</button></div>
    </form>
  </DialogShell>;
}
...
function PublishDialog({ detail, tenant, onClose, onChanged }: { detail: DocumentDetailResponse; tenant: string; onClose: () => void; onChanged: () => void }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : "Publication could not be completed.");
    } finally {
      setBusy(false);
    }
  };

  return <DialogShell title="Publish controlled revision" description="Publication is immutable. Eligible users will be notified automatically and acknowledgement tasks will be created when required." busy={busy} onClose={onClose}>
    <form className="dc-form" onSubmit={submit}>
