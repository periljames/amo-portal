...
function RevisionUploadDialog({ detail, tenant, onClose, onChanged }: { detail: DocumentDetailResponse; tenant: string; onClose: () => void; onChanged: () => void }) {
  ...
  return <DialogShell title="Upload a controlled revision" description="Create a working draft, or register the exact final PDF when an authority approval already exists." busy={busy} onClose={onClose}><form className="dc-form" onSubmit={submit}>
    ...
    <label><span>Intake state</span><select value={uploadState} onChange={(event) => setUploadState(event.target.value as UploadState)}><option value="DRAFT">Controlled draft (not issued)</option><option value="APPROVED" disabled={preview?.source_type !== "PDF"}>Already approved final PDF</option></select><small>{preview?.source_type !== "PDF" ? "Select the final approved PDF to enable this option; DOCX files follow the review workflow." : "Record the existing approval reference below."}</small></label>
