...
const AdminAmoAssetsPage: React.FC = () => {
  ...
  const effectiveAmoId = useMemo(() => {
    if (!currentUser?.amo_id) return activeAmoId;
    return isSuperuser ? activeAmoId || currentUser.amo_id : currentUser.amo_id;
  ...
  const syncContext = useCallback(async () => {
    if (isSuperuser && effectiveAmoId) {
      await setAdminContext({ active_amo_id: effectiveAmoId });
    }
  }, [effectiveAmoId, isSuperuser]);

  const loadSetup = useCallback(async (mode: "initial" | "refresh" = "refresh") => {
    if (!currentUser || !canAccessAdmin || !effectiveAmoId) return;
    mode === "initial" ? setLoading(true) : setRefreshing(true);
    setError(null);
    try {
      await syncContext();
      const [assetResult, baseResult, departmentResult, userResult, workforceResult, identityResult] = await Promise.allSettled([
        getAmoAssets(isSuperuser ? effectiveAmoId : null),
        listBaseStations({ include_inactive: true }),
        listAdminDepartments(isSuperuser ? effectiveAmoId : undefined),
      ...
      else failures.push("branding assets");
      if (baseResult.status === "fulfilled") setBases(baseResult.value);
      else failures.push("operating bases");
      ...
      else failures.push("workforce readiness");
      if (identityResult.status === "fulfilled") setIdentityHealth(identityResult.value);
      else failures.push("personnel identity health");

      if (failures.length) {
        setError(`Some setup data could not be loaded: ${failures.join(", ")}. The available sections remain usable.`);
      }
    } catch (cause) {
  ...
  }, [canAccessAdmin, currentUser, effectiveAmoId, isSuperuser, syncContext]);
  ...
  useEffect(() => {
    const requestedTour = new URLSearchParams(location.search).get("tour") === "1";
    const seen = localStorage.getItem(setupStorageKey(effectiveAmoId)) === "complete";
    if (requestedTour || !seen) setTourOpen(true);
  }, [effectiveAmoId, location.search]);

  useEffect(() => {
    const section = (new URLSearchParams(location.search).get("section") || "") as SetupSection;
    const target = section === "bases" ? basesRef.current
      : section === "assets" ? assetsRef.current
  ...
  useEffect(() => () => {
    if (previewAsset?.url) window.URL.revokeObjectURL(previewAsset.url);
  }, [previewAsset]);
      ...
      <div className="admin-page admin-amo-assets setup-centre">
        ...
        <div className="setup-centre__toolbar">
          ...
          <div className="setup-centre__toolbar-actions">
            ...
            <Button type="button" variant="secondary" disabled={refreshing} onClick={() => void loadSetup("refresh")}><RefreshCw size={16} className={refreshing ? "is-spinning" : ""} /> Refresh</Button>
          </div>
        </div>
          ...
          <div className="setup-centre__support-context">
            ...
            <select
              ...
              onChange={(event) => {
                ...
                localStorage.setItem(LS_ACTIVE_AMO_ID, value);
                setActiveAmoId(value);
              }}
            >
        ...
        <section ref={readinessRef} className="setup-centre__section" id="setup-readiness">
          <div className="setup-centre__section-heading">
            <div><span>Live readiness</span><h2>What must be configured</h2><p>Every status below is calculated from the current AMO data. Buttons open the exact page or editor needed to resolve the gap.</p></div>
          </div>
        ...
        <section ref={basesRef} className="setup-centre__section" id="setup-bases">
          <div className="setup-centre__section-heading">
            <div><span>Operating structure</span><h2>Bases, stations and facilities</h2><p>These canonical records feed Workforce contracts, user assignments, rostering, planning and maintenance execution. Do not duplicate base names inside individual modules.</p></div>
        ...
        <section ref={assetsRef} className="setup-centre__section" id="setup-assets">
          <div className="setup-centre__section-heading">
            <div><span>Controlled release output</span><h2>CRS branding and template</h2><p>Upload the approved logo and PDF template used by the certificate-of-release workflow. Replacing a file keeps the latest active version.</p></div>
          </div>
          ...
          {previewAsset ? (
            <div className="setup-centre__preview">
              <div><strong>{previewAsset.name}</strong><button type="button" aria-label="Close preview" onClick={() => setPreviewAsset(null)}><X size={17} /></button></div>
        ...
        <section ref={nextRef} className="setup-centre__section" id="setup-next">
          <div className="setup-centre__section-heading">
            <div><span>Continue configuration</span><h2>Module-specific setup pages</h2><p>Use these admin-only destinations after the core operating structure is ready. Each action routes to a working page rather than a decorative button.</p></div>
          </div>
