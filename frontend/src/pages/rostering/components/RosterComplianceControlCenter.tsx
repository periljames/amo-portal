  ...
  return (
    <details className="wr-native-guidance">
      ...
      <section className="wr-panel">
        ...
        {canEdit ? <>
          <div className="wr-form-grid">
            <label className="wr-span-2"><span>Affected assignment</span><select value={assignmentId} onChange={(event) => setAssignmentId(event.target.value)}><option value="">Select duty</option>{dutyAssignments.map((row) => <option key={row.id} value={row.id}>{row.user_full_name || "Person unavailable"} · {row.shift_code || row.status} · {formatDateTime(row.starts_at)} → {formatDateTime(row.ends_at)}</option>)}</select></label>
