...
export function WorkforceBulkSetupPanel({ canManageContracts, canInitializeDefaults }: Props) {
  ...
  const patchOverride = (userId: string, patch: Partial<HrContractOverride>) => { setOverrides((old) => ({ ...old, [userId]: { user_id: userId, ...old[userId], ...patch } })); setContractPreview(null); };
  ...
  return <section className="wr-panel workforce-bulk">
    ...
    <div className="workforce-bulk__pager"><span>{total ? `${(page - 1) * pageSize + 1}-${Math.min(page * pageSize, total)} of ${total.toLocaleString()}` : "0 records"}</span><select value={pageSize} onChange={(e) => { setPageSize(Number(e.target.value) as typeof pageSize); setPage(1); }}>{PAGE_SIZES.map((x) => <option key={x}>{x} per page</option>)}</select><button type="button" disabled={page <= 1} onClick={() => setPage(page - 1)}>Previous</button><span>Page {page} of {Math.max(1, pages)}</span><button type="button" disabled={!pages || page >= pages} onClick={() => setPage(page + 1)}>Next</button></div>
