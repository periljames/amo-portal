...
export function ReliabilityAnalyticsOperationalCharts({ trendRows, eventMixRows, ataRows, aircraftRows, stationRows, routeRows, componentRows, removalAgeRows, shopVisitRows, oilRows, onPoint }: Props): React.ReactElement {
  return <>
    <ChartCard id="rel-chart-trend" eyebrow="Normalised trend" title="Fleet reliability over time" description="Event rate per 100 flight hours and dispatch reliability against recorded cycles." empty={trendRows.length === 0} wide onExport={() => exportChartSvg("rel-chart-trend", "reliability-fleet-trend")}>
    ...
    <ChartCard id="rel-chart-event-mix" eyebrow="Operational consequence" title="Technical interruption mix" description="Canonical event composition and accumulated delay burden." empty={eventMixRows.length === 0} onExport={() => exportChartSvg("rel-chart-event-mix", "reliability-event-mix")}>
      <ResponsiveContainer width="100%" height={CHART_HEIGHT}>
    ...
    <ChartCard id="rel-chart-ata" eyebrow="Pareto analysis" title="ATA concentration" description="Top ATA chapters with cumulative share, delay burden and repeat defects." empty={ataRows.length === 0} onExport={() => exportChartSvg("rel-chart-ata", "reliability-ata-pareto")}>
      <ResponsiveContainer width="100%" height={CHART_HEIGHT}>
    ...
    <ChartCard id="rel-chart-aircraft" eyebrow="Fleet comparison" title="Aircraft event rate" description="Normalised event rate exposes aircraft performance without rewarding low utilisation." empty={aircraftRows.length === 0} onExport={() => exportChartSvg("rel-chart-aircraft", "reliability-aircraft-rate")}>
      <ResponsiveContainer width="100%" height={CHART_HEIGHT}>
    ...
    <ChartCard id="rel-chart-station" eyebrow="Network impact" title="Delay burden by origin station" description="Technical interruptions, delay minutes and cancellations at the dispatch station." empty={stationRows.length === 0} onExport={() => exportChartSvg("rel-chart-station", "reliability-station-delay")}>
      <ResponsiveContainer width="100%" height={CHART_HEIGHT}>
    ...
    <ChartCard id="rel-chart-route" eyebrow="Route impact" title="Technical delay by route" description="Delay minutes, interruption volume and cancellations by origin-destination pair." empty={routeRows.length === 0} wide onExport={() => exportChartSvg("rel-chart-route", "reliability-route-delay")}>
      <ResponsiveContainer width="100%" height={CHART_HEIGHT}>
    ...
    <ChartCard id="rel-chart-components" eyebrow="Component reliability" title="Shop outcomes and removals" description="No-fault-found, confirmed failures and removal mix by part number. Fleet exposure per removal is shown only where measurable." empty={componentRows.length === 0} wide onExport={() => exportChartSvg("rel-chart-components", "reliability-component-outcomes")}>
      <ResponsiveContainer width="100%" height={CHART_HEIGHT}>
    ...
    <ChartCard id="rel-chart-shop-visits" eyebrow="Repair exposure" title="Component shop-visit trend" description="Controlled shop-visit records by analytical period." empty={shopVisitRows.length === 0} onExport={() => exportChartSvg("rel-chart-shop-visits", "reliability-shop-visit-trend")}>
      <ResponsiveContainer width="100%" height={CHART_HEIGHT}>
        <BarChart data={shopVisitRows}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="label" /><YAxis allowDecimals={false} /><Tooltip /><Bar dataKey="shop_visits" name="Shop visits" onClick={(value: unknown) => onPoint(value, "Shop visits")} /></BarChart>
      </ResponsiveContainer>
    </ChartCard>

    <ChartCard id="rel-chart-oil-consumption" eyebrow="Oil consumption" title="Engine oil-consumption rate" description="Latest and average quarts per flight hour by aircraft and engine position." empty={oilRows.length === 0} wide onExport={() => exportChartSvg("rel-chart-oil-consumption", "reliability-oil-consumption")}>
      <ResponsiveContainer width="100%" height={CHART_HEIGHT}>
