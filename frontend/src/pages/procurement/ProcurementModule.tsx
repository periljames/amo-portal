    ...
    <DepartmentLayout amoCode={amoCode} activeDepartment="procurement">
      <div className="proc-page">
        ...
        <main className="proc-content">
          ...
          {section === "sourcing" ? <Sourcing rfqs={filtered(data.rfqs, (item) => `${item.rfq_number} ${item.title} ${item.status}`)} quotes={filtered(data.quotes, (item) => `${item.quote_reference} ${item.status}`)} loading={loading} search={searchBox} openModal={openModal} linkDocument={linkDocument} act={act} /> : null}
          ...
          {section === "receiving" ? <Receiving items={filtered(data.receipts, (item) => `${item.receipt_number} ${item.status} ${item.delivery_note_number || ""}`)} loading={loading} search={searchBox} canQuality={canQuality} openModal={openModal} linkDocument={linkDocument} act={act} /> : null}
          {section === "suppliers" ? <Suppliers items={filtered(data.suppliers, (item) => `${item.supplier_code} ${item.legal_name} ${item.status} ${item.country || ""}`)} loading={loading} search={searchBox} openModal={openModal} linkDocument={linkDocument} act={act} /> : null}
          {section === "control" ? <Control holds={data.holds} orders={data.orders} receipts={quarantine} canQuality={canQuality} canFinance={canFinance} openModal={openModal} linkDocument={linkDocument} act={act} /> : null}
          {section === "documents" ? <ProcurementDocumentCenter amoCode={amoCode} records={{ requisitions: data.requisitions, rfqs: data.rfqs, quotes: data.quotes, orders: data.orders, receipts: data.receipts, suppliers: data.suppliers, holds: data.holds }} initialEntity={documentTarget} canQuality={canQuality} canControl={canDocumentControl} /> : null}
        </main>
