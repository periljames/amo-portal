  ...
  const renderPlan = () => (
    <div className="tos-stack">
      ...
      <section className="tos-card">
        ...
        {nextBatch ? <div className="tos-table-wrap"><table className="tos-table"><thead><tr><th>Ranked person</th><th>Department</th><th>Due</th><th>Status / reason</th><th>Booking</th><th>Availability</th><th>Eligible</th></tr></thead><tbody>{nextBatch.candidates.map((candidate) => <tr key={candidate.user_id}><td>{candidate.full_name}<small>{candidate.department || ""}</small></td><td>{candidate.department || "—"}</td><td>{candidate.due_date || "Never completed"}<small>{candidate.days_remaining == null ? "" : `${candidate.days_remaining} day(s)`}</small></td><td><StatusPill value={candidate.status} /><small>{candidate.rank_reason}</small></td><td>{candidate.existing_booking || "None"}</td><td>{candidate.availability_conflict || "Available"}</td><td><StatusPill value={candidate.eligible ? "ELIGIBLE" : "CONFLICT"} /></td></tr>)}</tbody></table></div> : null}
      </section>
