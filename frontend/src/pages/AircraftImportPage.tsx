...
const AircraftImportPage: React.FC = () => {
  ...
  const getRouteSlugForAmo = (amo: AdminAmoRead | undefined): string | null => {
    if (!amo) return null;
    const slug = (amo.login_slug || "").trim();
    if (slug) return slug;
    const code = (amo.amo_code || "").trim();
  ...
  const handleAmoChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const routeSlug = getRouteSlugForAmo(amo);
    if (routeSlug) {
      navigate(
        `/maintenance/${routeSlug}/${currentDepartment}/aircraft-import`
      );
    }
  };
  ...
  const selectedAmo = useMemo(
    () => amoOptions.find((amo) => amo.id === selectedAmoId),
    [amoOptions, selectedAmoId]
  );
      ...
      {userIsSuperuser && (
        <section className="page-section">
          ...
          <p className="page-section__body">
            ...
            platform superusers, and the app will route you to the correct AMO
            ...
            <div className="form-row">
              ...
              {selectedAmo && (
                <div className="form-hint">
                  Active AMO: <strong>{selectedAmo.name}</strong> ·{" "}
                  {selectedAmo.amo_code}
                </div>
              )}
