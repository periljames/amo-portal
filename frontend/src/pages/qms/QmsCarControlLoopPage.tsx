              ...
              <div className="form-grid">
                <label>Accountable lead owner
                  <select className="input" value={initOwner} onChange={(event) => setInitOwner(event.target.value)}>
                    ...
                    {assigneeOptions.map((person) => <option key={person.id} value={person.id}>{personDisplay(person.full_name || person.email)}{person.department_name ? ` · ${person.department_name}` : ""}</option>)}
                  </select>
                </label>
                ...
                <div className="form-grid">
                  <label>Accountable owner
                    <select className="input" value={profileOwner} onChange={(event) => setProfileOwner(event.target.value)}>
                      <option value="">Unassigned</option>
                      {assigneeOptions.map((person) => <option key={person.id} value={person.id}>{personDisplay(person.full_name || person.email)}</option>)}
                    </select>
                  </label>
                    ...
                    {control.milestones.map((milestone) => {
                      ...
                      return (
                        <tr key={milestone.id}>
                          ...
                          <td>{canManage ? <select className="input" value={draft.owner_user_id} onChange={(event) => setMilestoneDrafts((current) => ({ ...current, [milestone.id]: { ...draft, owner_user_id: event.target.value } }))}><option value="">Unassigned</option>{assigneeOptions.map((person) => <option key={person.id} value={person.id}>{personDisplay(person.full_name || person.email)}</option>)}</select> : assigneeName(assignees, milestone.owner_user_id)}</td>
            ...
            <section className="card">
              ...
              {canManage ? (
                <div className="form-grid">
                  ...
                  <label>Owner<select className="input" value={dependencyOwner} onChange={(event) => setDependencyOwner(event.target.value)}><option value="">Unassigned</option>{assigneeOptions.map((person) => <option key={person.id} value={person.id}>{personDisplay(person.full_name || person.email)}</option>)}</select></label>
