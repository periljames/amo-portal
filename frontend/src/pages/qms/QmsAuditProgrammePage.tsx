...
const QmsAuditProgrammePage: React.FC = () => {
  ...
  const transitionMutation = useMutation({
    ...
    onSuccess: (programme) => {
      ...
      // A committed transition response is the authoritative state for this
      // mutation. Do not immediately invalidate/refetch it: deployments may
      // read programme GETs through a replica/cache that can briefly lag the
      // writer, which previously let a successful UNDER_REVIEW/APPROVED/etc.
      // transition visibly snap back to the older state. Explicit refresh,
      // focus/mount revalidation or a later operation can fetch again once the
      // read path has caught up.
        ...
        <section className="qms-audit-programme__detail">
          {!selectedProgrammeId ? <p>Select or create a programme revision.</p> : detailQuery.isLoading ? <p>Loading programme…</p> : !selected ? <p>Programme not available.</p> : (
            <>
              ...
              <section className="qms-audit-programme__governance"><header><ShieldCheck size={17} /><div><strong>Governance</strong><small>State changes require an attributable reason.</small></div></header><textarea rows={3} value={actionReason} onChange={(event) => setActionReason(event.target.value)} placeholder="Reason for transition or amendment" />
