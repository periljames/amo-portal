  ...
  return (
    <DepartmentLayout amoCode={amoCode} activeDepartment="quality">
      <div className={`qms-register-page qms-register-page--${module.id}`}>
        <PageHeader compact eyebrow="Quality Management System" title={module.label} subtitle={isInbox ? `${viewLabel(view)}. Prioritise your assigned approvals, reviews, verifications and assurance work.` : `${viewLabel(view)} workspace. Results are server-bounded; open the governed record to investigate or act.`} breadcrumbs={[{ label: "Quality", to: qmsBasePath(amoCode) }, { label: module.navigationLabel }, { label: viewLabel(view) }]} actions={!controlledNew ? <button type="button" className="qms-register-refresh" onClick={() => void load(true)} disabled={state === "loading"}><RefreshCw size={16} className={state === "loading" ? "is-spinning" : ""} aria-hidden="true" /> Refresh</button> : null} />
