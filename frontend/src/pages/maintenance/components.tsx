...
const tabs = [
  ...
  ["Work Packages", "/maintenance/work-packages"],
  ["Defects", "/maintenance/defects"],
  ["Non-Routines", "/maintenance/non-routines"],
  ["Inspections", "/maintenance/inspections"],
  ["Parts/Tools", "/maintenance/parts-tools"],
  ["Closeout", "/maintenance/closeout"],
  ...
  return (
    <DepartmentLayout amoCode={amoCode} activeDepartment={department}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
        {tabs.map(([label, path]) => (
          <Link key={path} to={path} className={`btn ${location.pathname.startsWith(path) ? "btn-primary" : "btn-secondary"}`}>{label}</Link>
        ))}
      </div>
      {children}
    </DepartmentLayout>
