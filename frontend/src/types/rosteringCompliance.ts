...
export type RosterAssignmentConsentRead = {
  ...
  planned_end: string;
  original_schedule_json?: Record<string, unknown> | null;
  personnel_response: RosterConsentStatus;
  personnel_response_at?: string | null;
  personnel_comment?: string | null;
  supervisor_required: boolean;
  supervisor_user_id?: string | null;
  supervisor_decision: RosterSupervisorDecision;
  supervisor_decision_at?: string | null;
  supervisor_decided_by_user_id?: string | null;
  supervisor_comment?: string | null;
  overtime_rest_day_classification?: string | null;
  replacement_rest_json?: Record<string, unknown> | null;
  statutory_compliance_json?: Record<string, unknown> | null;
  fatigue_risk_json?: Record<string, unknown> | null;
...
export type RosterConsentPersonnelDecision = {
  decision: "ACCEPT" | "DECLINE";
  assignment_fingerprint: string;
  comment?: string | null;
};

export type RosterConsentSupervisorDecision = {
...
export type RosterWorkflowGateRead = {
  severity: RosterWorkflowGateSeverity;
  code: string;
  message: string;
...
export type RosterWorkflowGateResponse = {
  version_id: string;
  workflow_state: "STATUTORY_BLOCKED" | "AWAITING_WORKFLOW_ACTION" | "READY_WITH_WARNINGS" | "READY" | string;
  hard_block_count: number;
  conditional_block_count: number;
  warning_count: number;
  can_submit: boolean;
  can_approve: boolean;
  can_publish: boolean;
  gates: RosterWorkflowGateRead[];
};
...
export type RosterDutyExtensionRead = {
  ...
  reason: string;
  normal_duty_start: string;
  original_planned_end: string;
  proposed_extended_end: string;
  continuous_duty_minutes: number;
  required_recovery_rest_minutes: number;
  recovery_rest_basis?: string | null;
  compliance_snapshot_json: Record<string, unknown>;
  fatigue_risk_json: Record<string, unknown>;
