...
const LiveAuditWorkspace: React.FC<Props> = ({ amoCode, auditKey }) => {
  ...
  const counts = useMemo(() => {
    const base: Record<CanonicalChecklistResponse, number> = {
      COMPLIANT: 0,
      NONCOMPLIANT: 0,
      OBSERVATION: 0,
      NOT_APPLICABLE: 0,
  ...
  if (auditQuery.isError || checklistQuery.isError || !auditQuery.data) {
    ...
    return (
      <div className="qms-live-audit-focus qms-live-audit-focus--loading" role="alert">
        <AlertTriangle size={20} /> {error instanceof Error ? error.message : "Live audit workspace could not be loaded."}
      </div>
    );
  }
  ...
  return (
    <div className="qms-live-audit-focus" role="region" aria-label="Live audit fieldwork workspace">
      <header className="qms-live-audit-focus__header">
        <div>
          <span>LIVE AUDIT · governed fieldwork</span>
      ...
      <div className="qms-live-audit-focus__body">
          ...
          <div className="qms-live-audit-focus__question-list">
            {items.map((item, index) => (
              <button
                ...
                className={item.checklist_item_id === selected?.checklist_item_id ? "is-selected" : ""}
                onClick={() => setSelectedId(item.checklist_item_id)}
              >
                <span>{index + 1}</span>
          ...
          {selected ? (
            <>
              <div className="qms-live-audit-focus__question-head">
                <div><span>{selected.section || "Checklist"}</span><h2>{selected.prompt}</h2></div>
              ...
              {selected.evidence_references.length ? (
                <section className="qms-live-audit-focus__evidence">
                  <strong>Linked evidence</strong>
                  <ul>{selected.evidence_references.map((reference, index) => <li key={index}>{typeof reference === "string" ? reference : JSON.stringify(reference)}</li>)}</ul>
                </section>
              ) : null}
        ...
        <aside className="qms-live-audit-focus__summary">
          <section>
            ...
            <small>{completed} of {items.length} questions resolved</small>
          </section>
          <section className="qms-live-audit-focus__stats">
          ...
          <section>
            ...
            <ul>{findings.slice(0, 6).map((finding) => <li key={finding.id}><b>{finding.finding_ref || finding.level}</b><small>{finding.description}</small></li>)}</ul>
          </section>
