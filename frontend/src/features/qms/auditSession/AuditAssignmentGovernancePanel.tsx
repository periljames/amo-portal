...
function eligibilitySummary(row: AuditAssignmentEligibility | undefined): string {
  ...
  if (!row.governed) return row.eligible ? "Eligible under legacy fallback because People governance is not configured." : "Assignment blocked.";
  if (row.eligible) return "Eligible · privilege, training, independence and capacity gates satisfied.";
  return row.blockers.map((blocker) => blocker.message).join(" · ") || "Assignment blocked by governed eligibility.";
}
...
const AuditAssignmentGovernancePanel: React.FC<Props> = ({ amoCode, auditKey }) => {
  ...
  const [reason, setReason] = useState("Assign the governed audit team after privilege, training, independence and capacity verification.");
  ...
  const eligibilityQueries = ROLE_CONFIG.map(({ field, role }) => useQuery({
    queryKey: ["qms-audit-assignment-eligibility", amoCode, auditId, role, draft[field]],
    queryFn: ({ signal }) => getAuditAssignmentEligibility(amoCode, auditId, draft[field], role, signal),
    enabled: Boolean(auditId && draft[field]),
    staleTime: 1_500,
    retry: false,
  }));

  const eligibilityByRole = useMemo(() => new Map(ROLE_CONFIG.map((config, index) => [config.role, eligibilityQueries[index].data])), [eligibilityQueries]);
  ...
  const assignmentMutation = useMutation({
    ...
    onSuccess: async () => {
      setError(null);
      setNotice("Audit team committed through the governed assignment gate. The eligibility snapshot and decision reason are retained in the audit trail.");
      await Promise.all([
  ...
  const declarationMutation = useMutation({
    ...
    onSuccess: async () => {
      setError(null);
      setNotice("Audit-specific independence declaration recorded. Eligibility is being re-evaluated against the immutable declaration.");
  ...
  return (
    <article className="qms-occurrence-stage__card" aria-label="Governed audit team assignment">
      <header><UserCheck size={18} /><div><strong>Governed audit-team assignment</strong><small>People &amp; Privileges is authoritative for privilege, training, independence and workload capacity.</small></div></header>
      ...
      <div className="qms-occurrence-stage__fields">
        {ROLE_CONFIG.map(({ field, role, label }) => {
          ...
          return <label key={field}><span>{label}</span><select disabled={!canManage} value={draft[field]} onChange={(event) => setDraft((current) => ({ ...current, [field]: event.target.value }))}><option value="">Unassigned</option>{(personnelQuery.data || []).map((person) => <option key={person.id} value={person.id}>{person.full_name}{person.role ? ` · ${person.role}` : ""}</option>)}</select><small className={assessment && !assessment.eligible ? "is-error" : ""}>{eligibilitySummary(assessment)}</small>{assessment?.independence?.required ? <small>Independence: {assessment.independence.declaration || assessment.independence.state || "pending"}</small> : null}</label>;
        })}
      </div>
      ...
      {canManage ? <details className="qms-occurrence-stage__independence"><summary>Record audit-specific independence declaration</summary><p>Use this only after the named person has made the declaration. A conflict or review-required declaration remains visible to the assignment gate and will block automatic eligibility.</p><div className="qms-occurrence-stage__fields"><label><span>Person</span><select value={declaration.userId} onChange={(event) => setDeclaration((current) => ({ ...current, userId: event.target.value }))}><option value="">Select person</option>{(personnelQuery.data || []).map((person) => <option key={person.id} value={person.id}>{person.full_name}</option>)}</select></label><label><span>Declaration</span><select value={declaration.declaration} onChange={(event) => setDeclaration((current) => ({ ...current, declaration: event.target.value as DeclarationDraft["declaration"] }))}><option value="INDEPENDENT">Independent</option><option value="CONFLICT">Conflict</option><option value="REQUIRES_REVIEW">Requires review</option></select></label></div><label><span>Relationship to audit subject</span><textarea rows={2} value={declaration.relationship} onChange={(event) => setDeclaration((current) => ({ ...current, relationship: event.target.value }))} placeholder="Required when a conflict exists" /></label><label><span>Declaration rationale</span><textarea rows={3} value={declaration.rationale} onChange={(event) => setDeclaration((current) => ({ ...current, rationale: event.target.value }))} /></label><button type="button" disabled={!declaration.userId || declaration.rationale.trim().length < 8 || declarationMutation.isPending || (declaration.declaration === "CONFLICT" && !declaration.relationship.trim())} onClick={() => declarationMutation.mutate()}>{declarationMutation.isPending ? "Recording…" : "Record declaration"}</button></details> : null}
    </article>
  );
};
