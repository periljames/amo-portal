...
const AuditFollowUpWorkspace: React.FC<Props> = ({ amoCode, auditKey }) => {
  ...
  if (loadError || !auditQuery.data || !closure) return <section className="qms-occurrence-stage qms-occurrence-stage--loading" role="alert"><AlertTriangle size={18} /> {loadError instanceof Error ? loadError.message : "Audit follow-up is unavailable."}</section>;

  return (
    <section className="qms-occurrence-stage" aria-label="Audit follow-up workspace">
      <header className="qms-occurrence-stage__header">
        <div><span>FOLLOW-UP · CAR / CAPA control loop</span><h1>{auditQuery.data.audit_ref} · {auditQuery.data.title}</h1><p>Execution closure does not close corrective action. This stage follows CAR ownership, milestones, extensions, escalation and effectiveness to their own governed completion.</p></div>
        <div><span>{closure.follow_up_status}</span><button type="button" onClick={() => void refresh()}><RefreshCw size={15} /> Refresh</button></div>
      </header>
      ...
      <div className="qms-occurrence-stage__grid">
        <main>
          ...
          {selectedCar ? <article className="qms-occurrence-stage__card">
            <header><ShieldAlert size={18} /><div><strong>{selectedCar.car_number} · control state</strong><small>{selectedCar.summary || selectedCar.title}</small></div></header>
        ...
        <aside>
          ...
          <article className="qms-occurrence-stage__card">
            <header><ExternalLink size={18} /><div><strong>Archive handoff</strong><small>Archive remains blocked until lifecycle and retention governance agree.</small></div></header>
