...
import { getActiveTenantAccessProfiles } from "../../services/accessProfiles";
import {
...
const newRule = () => ({ course_pk: "", scope: "ALL", department_code: "", access_profile_id: "", job_role: "", user_id: "", effective_from: new Date().toISOString().slice(0, 10), effective_to: "", manual_reference: "", planning_lead_days: "45", assessment_required: false, certificate_required: true, authorization_relevance: "", source_type: "MANUAL", source_id: "", blocking: false, required_by_date: "" });
...
const TrainingRequirementsWorkspace: React.FC<Props> = ({ canManage, onOpenImport }) => {
  ...
  const accessProfiles = useQuery({ queryKey: ["tenant-access-profile-options"], queryFn: getActiveTenantAccessProfiles, enabled: ruleOpen && rule.scope === "JOB_ROLE" });
  ...
  return <div className="tos-stack training-route-workspace">
    ...
    <details className="tos-disclosure" open><summary><span><Network size={18} /><strong>Requirement revisions</strong></span><small>{requirements.data?.length ?? "Unknown"} effective and historical rules</small></summary><div className="tos-disclosure__body"><div className="tos-list">{(requirements.data || []).map((item) => <div key={item.id}><div><strong>{courseById.get(item.course_id)?.course_id || item.course_id} · {item.scope}</strong><small>{item.department_code || item.access_profile_name || item.job_role || (item.user_id ? "Selected person" : "All active personnel")} · {item.manual_reference || "No manual reference"}{item.source_type ? ` · ${item.source_type}:${item.source_id || "unresolved"}` : ""}</small></div><span className={`tos-pill ${item.blocking ? "tos-pill--warning" : "tos-pill--ok"}`}>{item.blocking ? "BLOCKING" : item.is_active === false ? "RETIRED" : "EFFECTIVE"}</span></div>)}</div></div></details>
    ...
    <Drawer title="Create requirement revision" isOpen={ruleOpen} onClose={() => setRuleOpen(false)} panelClassName="training-form-drawer">
      <div className="tos-drawer-form">
        <label>Search course<input value={selectorSearch} onChange={(event) => setSelectorSearch(event.target.value)} /></label>
        <label>Course<select value={rule.course_pk} onChange={(event) => setRule({ ...rule, course_pk: event.target.value })}><option value="">Select course</option>{(selectableCourses.data || []).map((item) => <option key={item.id} value={item.id}>{item.course_id} · {item.course_name}</option>)}</select></label>
        <label>Applies to<select value={rule.scope} onChange={(event) => setRule({ ...rule, scope: event.target.value, department_code: "", access_profile_id: "", job_role: "", user_id: "" })}><option>ALL</option><option>DEPARTMENT</option><option>JOB_ROLE</option><option>USER</option></select></label>
        {rule.scope === "DEPARTMENT" ? <label>Department code<input value={rule.department_code} onChange={(event) => setRule({ ...rule, department_code: event.target.value.toUpperCase() })} /></label> : null}
        {rule.scope === "JOB_ROLE" ? <label>Tenant access profile<select value={rule.access_profile_id} onChange={(event) => setRule({ ...rule, access_profile_id: event.target.value })}><option value="">Select active profile</option>{(accessProfiles.data || []).map((profile) => <option key={profile.id} value={profile.id}>{profile.display_name} · {profile.category.replaceAll("_", " ")}</option>)}</select><small>The stable profile identity is used; later terminology changes will not break this rule.</small></label> : null}
        {rule.scope === "USER" ? <><label>Find portal user<input value={personSearch} onChange={(event) => setPersonSearch(event.target.value)} placeholder="Name, staff code or role" /></label><label>Canonical portal user<select value={rule.user_id} onChange={(event) => setRule({ ...rule, user_id: event.target.value })}><option value="">Select existing user</option>{(people.data || []).map((person) => <option key={person.id} value={person.id}>{person.full_name} · {person.staff_code || person.id}</option>)}</select><small>This links the requirement to User Management; no training-only profile is created.</small></label></> : null}
        <div className="tos-form-grid"><label>Effective from<input type="date" value={rule.effective_from} onChange={(event) => setRule({ ...rule, effective_from: event.target.value })} /></label><label>Effective to<input type="date" value={rule.effective_to} onChange={(event) => setRule({ ...rule, effective_to: event.target.value })} /></label><label>Planning lead days<input type="number" min="1" value={rule.planning_lead_days} onChange={(event) => setRule({ ...rule, planning_lead_days: event.target.value })} /></label><label>Controlled source<select value={rule.source_type} onChange={(event) => setRule({ ...rule, source_type: event.target.value, source_id: "" })}><option>MANUAL</option><option>DMS</option><option>QMS</option></select></label></div>
        {["DMS", "QMS"].includes(rule.source_type) ? <label>Search controlled source<input value={sourceSearch} onChange={(event) => setSourceSearch(event.target.value)} /><select value={rule.source_id} onChange={(event) => setRule({ ...rule, source_id: event.target.value })}><option value="">Select matching source</option>{(sourceResults.data || []).map((item) => <option key={item.id} value={item.id}>{item.code} · {item.label}</option>)}</select></label> : null}
        <label>Manual reference<input value={rule.manual_reference} onChange={(event) => setRule({ ...rule, manual_reference: event.target.value })} /></label>
        <label>Authorization relevance<textarea value={rule.authorization_relevance} onChange={(event) => setRule({ ...rule, authorization_relevance: event.target.value })} /></label>
        <label><input type="checkbox" checked={rule.blocking} onChange={(event) => setRule({ ...rule, blocking: event.target.checked })} /> Block completion/publication while the controlled source gate is open</label>
        <div className="tos-actions"><button onClick={() => setRuleOpen(false)}>Cancel</button><button className="primary-chip-btn" disabled={busy || !rule.course_pk || (rule.scope === "DEPARTMENT" && !rule.department_code) || (rule.scope === "JOB_ROLE" && !rule.access_profile_id) || (rule.scope === "USER" && !rule.user_id)} onClick={() => void saveRule()}>Save requirement</button></div>
      </div>
    </Drawer>
  </div>;
};
