...
import { Clock3, Download, FileBarChart, Plus, RefreshCw, RotateCcw } from "lucide-react";
import { useQuery, useQueryClient } from "@tanstack/react-query";

import Drawer from "../shared/Drawer";
import { listTrainingEvents } from "../../services/training";
import { createTrainingReportDefinition, downloadTrainingOperatingReport, downloadTrainingReportJob, listTrainingBudgets, listTrainingPlanSummaries, listTrainingReportDefinitions, listTrainingReportJobs, queueTrainingReportJob, retryTrainingReportJob } from "../../services/trainingOperating";
...
const TrainingReportsWorkspace: React.FC<Props> = ({ canManage, canExport }) => {
  ...
  const jobs = useQuery({ queryKey: ["training", "report-jobs"], queryFn: () => listTrainingReportJobs(), refetchInterval: 4_000 });
  ...
  const retry = async (jobId: string) => {
    setBusy(true); setError(null);
    try { await retryTrainingReportJob(jobId); await jobs.refetch(); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "Report job could not be retried."); } finally { setBusy(false); }
  };
