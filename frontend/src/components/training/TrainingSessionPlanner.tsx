...
const TrainingSessionPlanner: React.FC<Props> = ({ canManage, onOpenAttendance }) => {
  ...
  const [closeoutEvent, setCloseoutEvent] = useState<TrainingEventRead | null>(null);
  ...
  return <div className="tos-stack training-route-workspace">
    <section className="tos-card tos-route-commandbar"><div><p className="tos-kicker">Delivery lifecycle</p><h2>Session scheduler</h2><p>Create the governed event, build its canonical roster, deliver invitations and move directly to attendance, close-out and independent certification.</p></div><button disabled={!canManage} onClick={() => setSessionOpen(true)}><CalendarPlus size={16} /> Session</button><button onClick={() => void events.refetch()} aria-label="Refresh sessions"><RefreshCw size={16} /></button></section>
    {error ? <div className="tos-banner tos-banner--error">{error}<button onClick={() => setError(null)}>×</button></div> : null}
    <section className="tos-card tos-process-rail"><span>Plan cohort</span><i>→</i><span>Create session</span><i>→</i><span>Invite / RSVP</span><i>→</i><span>QR attendance</span><i>→</i><span>Evidence close-out</span><i>→</i><span>Independent verification</span></section>
    <section className="tos-card tos-register-card">{events.isError ? <div className="tos-empty"><strong>Session source unavailable</strong><span>No empty register is being inferred.</span></div> : <div className="tos-table-wrap"><table className="tos-table tos-table--interactive"><thead><tr><th>Date</th><th>Session</th><th>Provider / location</th><th>Roster</th><th>Status</th><th>Actions</th></tr></thead><tbody>{(events.data?.items || []).map((event) => <tr key={event.id}><td>{event.starts_on}<small>{event.ends_on && event.ends_on !== event.starts_on ? `to ${event.ends_on}` : ""}</small></td><td><strong>{event.title}</strong><small>{event.course_code || event.course_name || event.course_id}</small></td><td>{event.provider || "Internal"}<small><MapPin size={12} /> {event.location || "Venue not set"}</small></td><td>{event.participant_count ?? "Open roster"}</td><td><span className="tos-pill tos-pill--ok">{event.status}</span></td><td><div className="tos-actions"><button onClick={() => setRosterEvent(event)}><UsersRound size={15} /> Roster</button><button onClick={() => onOpenAttendance(event.id)}>Attendance</button><button disabled={!canManage} onClick={() => setCloseoutEvent(event)}>Close-out</button></div></td></tr>)}</tbody></table></div>}<footer className="tos-pagination"><span>Page {Math.floor(offset / PAGE_SIZE) + 1}</span><div><button disabled={!offset} onClick={() => setOffset(Math.max(0, offset - PAGE_SIZE))}><ChevronLeft size={17} /></button><button disabled={!events.data?.hasMore} onClick={() => setOffset(offset + PAGE_SIZE)}><ChevronRight size={17} /></button></div></footer></section>
