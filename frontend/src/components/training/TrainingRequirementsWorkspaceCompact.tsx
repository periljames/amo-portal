...
const newRule = () => ({
  course_pk: "", scope: "ALL", department_code: "", access_profile_id: "", job_role: "", user_id: "",
...
function requirementTarget(item: TrainingRequirementRead): string {
  ...
  if (item.scope === "JOB_ROLE") return item.access_profile_name || item.job_role || "Access profile not set";
...
const TrainingRequirementsWorkspaceCompact: React.FC<Props> = ({ canManage, onOpenImport }) => {
  ...
  const accessProfiles = useQuery({
    queryKey: ["tenant-access-profile-options"],
    queryFn: getActiveTenantAccessProfiles,
    enabled: ruleOpen && rule.scope === "JOB_ROLE",
  });
  ...
  const targetReady = rule.scope === "ALL"
    ...
    || (rule.scope === "JOB_ROLE" && Boolean(rule.access_profile_id))
  ...
  const saveRule = async () => {
    ...
    try {
      await createTrainingRequirement({
        ...
        access_profile_id: rule.scope === "JOB_ROLE" ? rule.access_profile_id : null,
        job_role: null,
  ...
  return <div className="tos-stack training-route-workspace">
    ...
    <Drawer title="Create requirement revision" isOpen={ruleOpen} onClose={() => setRuleOpen(false)} panelClassName="training-form-drawer">
      <div className="tos-drawer-form">
        ...
        <div className="tos-actions"><button onClick={() => setRuleOpen(false)}>Cancel</button><button className="primary-chip-btn" disabled={busy || !rule.course_pk || !targetReady} onClick={() => void saveRule()}>Save requirement</button></div>
      </div>
    </Drawer>
  </div>;
};
