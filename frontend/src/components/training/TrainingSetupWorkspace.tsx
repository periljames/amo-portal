...
import TrainingAssessmentPolicyRegister from "./TrainingAssessmentPolicyRegister";
...
const assessmentDraft = () => ({
  code: "", name: "", assessment_type: "WRITTEN", outcome_scheme: "NUMERIC", pass_threshold: "",
    ...
    <div className="tos-stack tos-setup-workspace">
      ...
      <details className="tos-disclosure" id="assessment-templates">
        ...
        <div className="tos-disclosure__body">
          <div className="tos-section-heading"><p>Questions and mandatory criteria are stored with each controlled template revision.</p><button disabled={!canManage} onClick={() => setAssessmentOpen(true)}><Plus size={15} /> New template</button></div>
          <div className="tos-list">{templates.map((template) => <div key={template.id}><div><strong>{template.code} · {template.name}</strong><small>{template.assessment_type} · Rev {template.revision_no} · {template.questions?.length || 0} criteria</small></div><span className="tos-pill tos-pill--ok">{template.active ? "ACTIVE" : "INACTIVE"}</span></div>)}</div>
          <TrainingAssessmentPolicyRegister templates={templates} canManage={canManage} onChanged={onChanged} />
        </div>
      </details>
