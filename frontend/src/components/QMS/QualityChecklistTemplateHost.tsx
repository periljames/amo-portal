  ...
  const body = (
    <div className="qms-checklist-template-body">
      ...
      {selectedTemplateId ? <section className="qms-checklist-template-card">
        ...
        {revisions.length ? <ol>{revisions.slice().sort((a, b) => b.revision_no - a.revision_no).map((row) => <li key={row.id}><strong>Rev {row.revision_no} · {row.status}</strong><span>{row.items.length} item(s) · {row.items.filter((item) => item.mandatory ?? true).length} mandatory</span><small>{row.change_reason}</small></li>)}</ol> : <p>No revisions are recorded for the selected template.</p>}
      </section> : null}
      ...
      {auditChecklist ? <section className="qms-checklist-template-card">
        ...
        {bindingsQuery.data?.items.length ? <ul>{bindingsQuery.data.items.map((row) => <li key={row.id}><strong>{row.template_code} Rev {row.revision_no}</strong><span>{row.instantiated_item_ids.length} checklist row(s)</span><small>{row.application_reason}</small></li>)}</ul> : <p>No checklist revision has been applied to this audit yet.</p>}
      </section> : null}
    </div>
  );
