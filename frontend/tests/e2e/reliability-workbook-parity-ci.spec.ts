  ...
  if (path.includes("/reliability/workbook-parity")) {
    ...
    if (path.endsWith("/oos-metrics")) {
      return json({
        records: 3,
        downtime_hours: 18.5,
        scheduled_available_hours: 720,
        available_hours: 701.5,
        availability_pct: 97.43,
        mttr_hours: 6.17,
      });
    }
    ...
    if (path.endsWith("/parity")) {
      return json(DATASETS.map((code) => ({
        dataset_code: code,
        dataset_name: names[code],
        required_fields: [],
        optional_fields: [],
    ...
    if (path.endsWith("/contracts")) {
      return json({
        ...
        report_layouts: { required_datasets: DATASETS, layouts: {} },
      });
    }
...
async function openWorkspace(page: Page): Promise<void> {
  ...
  await page.addInitScript(({ storedToken }) => {
    ...
    localStorage.setItem("amo_current_user", JSON.stringify({
      id: "uat-user",
      amo_id: "uat-amo",
      department_id: "uat-rel",
      staff_code: "REL-UAT",
      email: "uat@example.invalid",
      first_name: "Reliability",
      last_name: "UAT",
      full_name: "Reliability UAT",
      role: "QUALITY_MANAGER",
      position_title: "Reliability Manager",
      phone: null,
  ...
  try {
    await expect(
      page.getByTestId("reliability-workbook-parity"),
      `Expected workbook parity workspace at ${page.url()}`,
    ).toBeVisible({ timeout: 30_000 });
  } catch (error) {
    const body = (await page.locator("body").innerText().catch(() => "<body unavailable>")).slice(0, 4000);
    throw new Error(
      `Workbook parity workspace did not render. URL: ${page.url()}\n` +
...
test.describe("Reliability workbook parity representative-tenant UAT", () => {
  test("wires every register, governed analysis and controlled reports", async ({ page }) => {
