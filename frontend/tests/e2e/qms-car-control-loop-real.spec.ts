...
async function submitResponsibleManagerResponse(page: Page, data: Fixture, suffix: string): Promise<void> {
  ...
  await page.getByLabel("Immediate containment action").fill(`The sampled local CAR index was isolated and reconciled to the governed register ${suffix}.`);
  await page.getByRole("button", { name: "Save containment and continue" }).click();
  await page.getByLabel("Root cause analysis").fill(`The local workflow lacked an explicit effectiveness checkpoint and evidence-index ownership control ${suffix}.`);
  await page.getByRole("button", { name: "Save root cause and continue" }).click();
  await page.getByLabel("Corrective action plan").fill(`Add governed RCA/CAPA milestones, assign accountable owners, index evidence and require effectiveness verification before closure ${suffix}.`);
  await page.getByLabel("Preventive action / systemic control").fill(`Trend CAR effectiveness and overdue milestones in the Quality operating review ${suffix}.`);
